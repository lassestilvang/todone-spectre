...
export class CollaborationService {
  ...
  private validateTeam(teamData: Partial<CollaborationTeam>): void {
    ...
    if (
      teamData.privacySetting &&
      !["public", "private", "team-only"].includes(teamData.privacySetting)
    ) {
  ...
  private validateMember(memberData: Partial<CollaborationMember>): void {
    ...
    if (
      memberData.role &&
      !["admin", "member", "guest"].includes(memberData.role)
    ) {
    ...
    if (
      memberData.status &&
      !["active", "inactive", "pending"].includes(memberData.status)
    ) {
   ...
   */
  async createTeam(
    teamData: Omit<CollaborationTeam, "id" | "createdAt" | "updatedAt">,
    ...
    try {
      ...
      // Call API
      const response: ApiResponse<CollaborationTeam> =
        await collaborationApi.createTeam(newTeam);
  ...
  async getTeam(teamId: string): Promise<CollaborationTeam> {
    try {
      const response: ApiResponse<CollaborationTeam> =
        await collaborationApi.getTeam(teamId);
  ...
  async getTeams(): Promise<CollaborationTeam[]> {
    try {
      const response: ApiResponse<CollaborationTeam[]> =
        await collaborationApi.getTeams();
  ...
  async updateTeam(
    ...
    try {
      ...
      // Call API
      const response: ApiResponse<CollaborationTeam> =
        await collaborationApi.updateTeam(teamId, updates);
  ...
  async deleteTeam(teamId: string, confirm: boolean = true): Promise<void> {
    ...
    try {
      ...
      const response: ApiResponse<void> =
        await collaborationApi.deleteTeam(teamId);
  ...
  async addMemberToTeam(
    ...
    try {
      const response: ApiResponse<CollaborationMember> =
        await collaborationApi.addMemberToTeam(teamId, newMember);
  ...
  async removeMemberFromTeam(teamId: string, memberId: string): Promise<void> {
    try {
      const response: ApiResponse<void> =
        await collaborationApi.removeMemberFromTeam(teamId, memberId);
  ...
  async updateMemberRole(
    ...
    try {
      const response: ApiResponse<CollaborationMember> =
        await collaborationApi.updateMemberRole(teamId, memberId, newRole);
  ...
  async getTeamMembers(teamId: string): Promise<CollaborationMember[]> {
    try {
      const response: ApiResponse<CollaborationMember[]> =
        await collaborationApi.getTeamMembers(teamId);
  ...
  async updateTeamSettings(
    ...
    try {
      const response: ApiResponse<CollaborationSettings> =
        await collaborationApi.updateTeamSettings(teamId, settings);
  ...
  async getTeamSettings(teamId: string): Promise<CollaborationSettings> {
    try {
      const response: ApiResponse<CollaborationSettings> =
        await collaborationApi.getTeamSettings(teamId);
  ...
  filterTeamsByPrivacy(
    ...
    return this.collaborationStore.teams.filter(
      (team) => team.privacySetting === privacy,
    );
  }
