...
class AccessibilityService {
  ...
  private loadPreferences(): AccessibilityPreferences {
    try {
      const savedPrefs = localStorage.getItem('todone-accessibility-preferences');
      if (savedPrefs) {
  ...
  private savePreferences(preferences: AccessibilityPreferences): void {
    try {
      localStorage.setItem('todone-accessibility-preferences', JSON.stringify(preferences));
    } catch (error) {
  ...
  toggleHighContrast(): void {
    ...
    this.updatePreferences({ preferredContrast: newValue ? 'high' : 'default' });
  ...
  setFontSize(size: string): void {
    ...
    if (validSizes.includes(size)) {
      ...
      document.documentElement.style.setProperty('--base-font-size', accessibilityUtils.getFontSizeValue(size));
    }
  }
  ...
  toggleReduceMotion(): void {
    ...
    this.updatePreferences({ preferredMotion: newValue ? 'reduced' : 'default' });
  ...
  removeFeature(featureId: string): void {
    this.updateState({ features: this.state.features.filter(f => f !== featureId) });
  }
  ...
  applySystemPreferences(): void {
    ...
    if (this.preferences.preferredContrast === 'auto' && systemPrefersContrast) {
      this.updateState({ isHighContrast: true });
    }

    if (this.preferences.preferredMotion === 'auto' && systemPrefersReducedMotion) {
  ...
  resetToDefaults(): void {
    ...
    // Reset DOM changes
    document.body.classList.remove('high-contrast-mode', 'reduce-motion', 'screen-reader-enhanced', 'keyboard-navigation-enhanced');
    document.body.removeAttribute('aria-live');
