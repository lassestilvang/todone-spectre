...
export class RecurringTaskService {
  ...
  async deleteRecurringTask(
    ...
    try {
      ...
      const recurringTask = this.taskStore.tasks.find(
        (task) => task.id === taskId,
      );
      if (!recurringTask) {
        throw new Error("Recurring task not found");
      ...
      const generatedInstances = this.taskStore.tasks.filter(
        (task) => task.customFields?.originalTaskId === taskId,
      );
  ...
  async regenerateRecurringInstances(
    ...
    const existingInstances = this.taskStore.tasks.filter(
      (t) => t.customFields?.originalTaskId === task.id && t.id !== task.id,
    );
  ...
  async completeRecurringInstance(instanceId: string): Promise<Task> {
    try {
      ...
      if (response.success && response.data) {
        ...
        const originalTask = this.taskStore.tasks.find(
          (task) => task.id === instance.customFields?.originalTaskId,
        );
        ...
        throw new Error(
          response.message || "Failed to complete recurring instance",
        );
      }
    } catch (error) {
  ...
  async generateNextRecurringInstance(task: Task): Promise<Task | null> {
    ...
    const instances = this.taskStore.tasks
      .filter((t) => t.customFields?.originalTaskId === task.id)
  ...
  private validateTask(taskData: Partial<Task>): void {
    ...
    if (
      taskData.priority &&
      !["P1", "P2", "P3", "P4"].includes(taskData.priority)
    ) {
  ...
  getRecurringTaskStats(taskId: string): {
    ...
    const futureInstances = instances
      .filter(
        (i) => !i.completed && i.dueDate && isAfter(i.dueDate, new Date()),
      )
      .sort(
        (a, b) => (a.dueDate?.getTime() || 0) - (b.dueDate?.getTime() || 0),
      );
    ...
    try {
      ...
      if (allTasks.success && allTasks.data) {
        return allTasks.data.filter(
          (task) => task.recurringPattern && task.projectId === projectId,
        );
      }
      return [];
    } catch (error) {
  ...
  async getRecurringTasksByPattern(pattern: RecurringPattern): Promise<Task[]> {
    try {
      ...
      if (allTasks.success && allTasks.data) {
        return allTasks.data.filter(
          (task) => task.recurringPattern === pattern,
        );
      }
      return [];
    } catch (error) {
  ...
  async getRecurringTasksByStatus(status: TaskStatus): Promise<Task[]> {
    try {
      ...
      if (allTasks.success && allTasks.data) {
        return allTasks.data.filter(
          (task) => task.recurringPattern && task.status === status,
        );
      }
      return [];
    } catch (error) {
   ...
   */
  async updateRecurringInstancesStatus(
    instanceIds: string[],
  ...
  async getRecurringTaskCompletionStats(): Promise<{
    ...
    try {
      ...
      const recurringTasks = allTasks.data.filter(
        (task) => task.recurringPattern,
      );
      const recurringInstances = allTasks.data.filter(
      ...
      return {
        ...
        completedInstances: recurringInstances.filter((i) => i.completed)
          .length,
  ...
  > {
    try {
      ...
      console.error(
        "Error getting recurring task pattern distribution:",
        error,
      );
      throw error;
    }
   ...
   */
  async createRecurringTaskTemplate(
    taskId: string,
    templateName: string,
  ): Promise<{
    templateId: string;
   ...
   */
  async getRecurringTaskTimeline(taskId: string): Promise<
    Array<{
      date: Date;
  ...
  async getRecurringTaskAnalytics(taskId: string): Promise<{
    ...
    try {
      ...
      const onTimeCompletions = completedInstances.filter(
        (instance) =>
      ...
      const onTimeCompletionRate =
        onTimeCompletions.length / completedInstances.length;
    ...
    try {
      ...
      return instances
        ...
        .map((instance) => ({
          ...
          instanceId: instance.id,
          title: instance.isGenerated
            ? `${instance.title} (Recurring)`
            : instance.title,
          status: instance.status,
          isGenerated: instance.isGenerated,
        }));
   ...
   */
  async getRecurringTaskGanttData(taskId: string): Promise<
    Array<{
      instanceId: string;
      title: string;
      startDate: Date;
