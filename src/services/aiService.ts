...
class AIService {
  ...
  constructor(
    apiKey: string,
    baseUrl: string = "https://api.ai-service.com/v1",
  ) {
    this.apiKey = apiKey;
  ...
  async analyzeTaskComplexity(
    task: Task,
  ): Promise<{ complexityScore: number; complexityLevel: string }> {
  ...
  private parseTaskBreakdown(response: string): {
    ...
    return {
      ...
      resources:
        resources.length > 0
          ? resources
          : ["Task management tools", "Documentation resources"],
    };
  }
}
...
const aiServiceInstance = new AIService(
  process.env.REACT_APP_AI_API_KEY || "default-api-key",
);
