...
import { addDays, addWeeks, addMonths, addYears, isBefore, isAfter, isEqual, parseISO, format, getDay, setDay } from 'date-fns';
...
export class RecurringPatternService {
   ...
   */
  private getNextDailyDate(currentDate: Date, config: RecurringPatternConfig): Date {
   ...
   */
  private getNextWeeklyDate(currentDate: Date, config: RecurringPatternConfig): Date {
    const interval = config.interval || 1;
   ...
   */
  private getNextMonthlyDate(currentDate: Date, config: RecurringPatternConfig): Date {
    const interval = config.interval || 1;
   ...
   */
  private getNextPositionalMonthlyDate(currentDate: Date, config: RecurringPatternConfig): Date {
   ...
   */
  private getNextYearlyDate(currentDate: Date, config: RecurringPatternConfig): Date {
    const interval = config.interval || 1;
   ...
   */
  private getNextCustomDate(currentDate: Date, config: RecurringPatternConfig): Date {
    if (config.frequency) {
  ...
  parsePatternConfigFromTask(taskCustomFields: Record<string, any>): RecurringPatternConfig {
    return {
      ...
      endDate: taskCustomFields.recurringEndDate ? new Date(taskCustomFields.recurringEndDate) : null,
      ...
      customMonthPosition: taskCustomFields.recurringCustomMonthPosition || null,
