...
interface UserPatternData {
  ...
  learningHistory: Array<{
    timestamp: Date;
    action: 'task_completed' | 'task_created' | 'priority_changed' | 'suggestion_accepted' | 'suggestion_rejected';
    taskId: string;
    context: any;
...
interface LearningInsight {
  insightId: string;
  insightType: 'productivity' | 'prioritization' | 'scheduling' | 'collaboration';
  description: string;
  confidence: number;
...
export class AIUserPatternService {
  ...
  public async getPersonalizedRecommendations(): Promise<string[]> {
    ...
    if (this.userPatterns.prioritizationPatterns.reprioritizationFrequency > 0.3) {
      recommendations.push(
  ...
  private async updateCompletionPatterns(task: Task, timestamp: Date) {
    ...
    if (this.userPatterns.taskCompletionPatterns.byDayOfWeek[dayOfWeek]) {
      const current = this.userPatterns.taskCompletionPatterns.byDayOfWeek[dayOfWeek];
      current.completionRate = (current.completionRate * current.avgTasks + 1) / (current.avgTasks + 1);
    ...
    if (this.userPatterns.taskCompletionPatterns.byTimeOfDay[timeSlot]) {
      const current = this.userPatterns.taskCompletionPatterns.byTimeOfDay[timeSlot];
      current.completionRate = (current.completionRate * current.avgTasks + 1) / (current.avgTasks + 1);
    ...
    if (this.userPatterns.taskCompletionPatterns.byTaskType[taskType]) {
      const current = this.userPatterns.taskCompletionPatterns.byTaskType[taskType];
    ...
    } else if (this.userPatterns.productivityPatterns.lowEnergyHours.includes(timeSlot)) {
      // If completing tasks during low energy hours, might need to reconsider
      const index = this.userPatterns.productivityPatterns.lowEnergyHours.indexOf(timeSlot);
      if (index > -1) {
  ...
  private async updateCreationPatterns(task: Task, timestamp: Date) {
    ...
    const taskType = this.inferTaskType(task);
    if (!this.userPatterns.productivityPatterns.preferredTaskTypes.includes(taskType) &&
        !this.userPatterns.productivityPatterns.avoidedTaskTypes.includes(taskType)) {
      // New task type - add to preferred if created during peak hours
      if (this.userPatterns.productivityPatterns.peakHours.includes(timeSlot)) {
        this.userPatterns.productivityPatterns.preferredTaskTypes.push(taskType);
      }
    }
    ...
    if (task.estimatedDuration) {
      this.userPatterns.schedulingPatterns.avgTaskDuration =
        (this.userPatterns.schedulingPatterns.avgTaskDuration * 4 + task.estimatedDuration) / 5;
    }
  }
  ...
  private async updatePrioritizationPatterns(task: Task, timestamp: Date) {
    ...
    this.userPatterns.prioritizationPatterns.reprioritizationFrequency =
      (this.userPatterns.prioritizationPatterns.reprioritizationFrequency * 9 + 1) / 10;
    ...
    if (task.priority === 'high') {
      ...
      this.userPatterns.prioritizationPatterns.urgencyResponseTime =
        (this.userPatterns.prioritizationPatterns.urgencyResponseTime * 3 + hoursSinceCreation) / 4;
    }
  }

  private async updateSuggestionPatterns(task: Task, accepted: boolean, timestamp: Date) {
    // Update confidence based on suggestion acceptance
    const taskType = this.inferTaskType(task);
    const currentConfidence = this.userPatterns.taskCompletionPatterns.byTaskType[taskType]?.completionRate || 0.7;

    if (accepted) {
      // Increase confidence for this task type
      this.userPatterns.taskCompletionPatterns.byTaskType[taskType].completionRate =
        Math.min(1.0, currentConfidence + 0.05);
    } else {
      // Decrease confidence for this task type
      this.userPatterns.taskCompletionPatterns.byTaskType[taskType].completionRate =
        Math.max(0.3, currentConfidence - 0.03);
    }
  }

  private async analyzeTaskCompletionPatterns(): Promise<PatternAnalysisResult> {
    const highestCompletionDay = Object.entries(this.userPatterns.taskCompletionPatterns.byDayOfWeek)
      .sort((a, b) => b[1].completionRate - a[1].completionRate)[0];

    const highestCompletionTime = Object.entries(this.userPatterns.taskCompletionPatterns.byTimeOfDay)
      .sort((a, b) => b[1].completionRate - a[1].completionRate)[0];

    const bestTaskType = Object.entries(this.userPatterns.taskCompletionPatterns.byTaskType)
      .sort((a, b) => b[1].completionRate - a[1].completionRate)[0];
  ...
  private async analyzeProductivityPatterns(): Promise<PatternAnalysisResult> {
    const peakHoursCount = this.userPatterns.productivityPatterns.peakHours.length;
    const lowEnergyCount = this.userPatterns.productivityPatterns.lowEnergyHours.length;
    ...
    return {
      ...
      data: {
        ...
        preferredTaskTypes: this.userPatterns.productivityPatterns.preferredTaskTypes
      }
    };
  }

  private async analyzePrioritizationPatterns(): Promise<PatternAnalysisResult> {
    const reprioritizationRate = this.userPatterns.prioritizationPatterns.reprioritizationFrequency;
    const urgencyResponse = this.userPatterns.prioritizationPatterns.urgencyResponseTime;
    ...
    return {
      ...
      recommendations: [
        ...
        reprioritizationRate > 0.3 ? 'Consider using AI prioritization to reduce frequent priority changes' : 'Your prioritization approach is stable'
      ],
      data: {
  ...
  private async analyzeSchedulingPatterns(): Promise<PatternAnalysisResult> {
    ...
    const bufferPreference = this.userPatterns.schedulingPatterns.bufferTimePreference;
    ...
    return {
      ...
      recommendations: [
        ...
        bufferPreference > 1.5 ? 'You prefer generous time buffers' : 'You work well with tight schedules',
        avgDuration > 2 ? 'Consider breaking larger tasks into smaller subtasks' : 'Your task sizing is optimal'
      ],
      data: {
        avgDuration,
        accuracy,
  ...
  private async analyzeCollaborationPatterns(): Promise<PatternAnalysisResult> {
    const delegationRate = this.userPatterns.collaborationPatterns.delegationFrequency;
    const teamworkPref = this.userPatterns.collaborationPatterns.teamworkPreference;
    const commStyle = this.userPatterns.collaborationPatterns.communicationStyle;
    ...
    return {
      ...
      recommendations: [
        ...
        delegationRate < 0.2 ? 'Consider delegating more tasks to team members' : 'Your delegation approach is balanced'
      ],
      data: {
        delegationRate,
        teamworkPref,
  ...
  private generateProductivityInsight(): LearningInsight | null {
    ...
    const lowEnergyHours = this.userPatterns.productivityPatterns.lowEnergyHours;
  ...
  private generatePrioritizationInsight(): LearningInsight | null {
    const reprioritizationFreq = this.userPatterns.prioritizationPatterns.reprioritizationFrequency;

    if (reprioritizationFreq < 0.1) return null;

    let recommendation = '';
    if (reprioritizationFreq > 0.4) {
      recommendation = 'You frequently change task priorities. Consider using the AI prioritization assistant for more stable planning.';
    } else if (reprioritizationFreq > 0.2) {
      recommendation = 'You occasionally adjust priorities. Review your planning approach for better stability.';
    } else {
      recommendation = 'Your prioritization approach is relatively stable. Keep up the good planning!';
    }
    ...
    return {
      ...
      supportingData: {
        ...
        priorityAdjustments: this.userPatterns.prioritizationPatterns.priorityAdjustments
      },
      timestamp: new Date()
    };
  }
  ...
  private generateSchedulingInsight(): LearningInsight | null {
    ...
    let recommendation = '';
    if (avgDuration > 3) {
      recommendation = 'You work on longer tasks. Consider breaking them into smaller, more manageable subtasks for better progress tracking.';
    } else if (avgDuration > 2) {
      recommendation = 'Your tasks are moderately sized. Review if some could be broken down for better focus.';
    } else {
      recommendation = 'Your task sizing is optimal for productivity. Maintain this approach.';
    }
    ...
    return {
      ...
      supportingData: {
        ...
        schedulingAccuracy: this.userPatterns.schedulingPatterns.schedulingAccuracy
      },
      timestamp: new Date()
    };
  }
  ...
  private generateCollaborationInsight(): LearningInsight | null {
    const delegationFreq = this.userPatterns.collaborationPatterns.delegationFrequency;

    if (delegationFreq < 0.05) return null;

    let recommendation = '';
    if (delegationFreq > 0.3) {
      recommendation = 'You delegate tasks frequently. Ensure you maintain a good balance between delegation and personal contribution.';
    } else if (delegationFreq > 0.15) {
      recommendation = 'You have a healthy delegation approach. Consider mentoring team members on some of your specialized tasks.';
    } else {
      recommendation = 'You could delegate more tasks to develop your team and free up your time for higher-value work.';
    }
    ...
    return {
      ...
      supportingData: {
        ...
        teamworkPreference: this.userPatterns.collaborationPatterns.teamworkPreference
      },
      timestamp: new Date()
    };
  }
  ...
  private calculateTaskDuration(task: Task): number {
    ...
    const durationHours = (completed.getTime() - created.getTime()) / (1000 * 3600);
  ...
  public async learnFromTaskCompletion(taskId: string, completionData: {
  ...
  private updateDurationPatterns(taskId: string, actualDuration: number) {
    ...
    const accuracy = 1 - Math.abs(actualDuration - estimatedDuration) / Math.max(estimatedDuration, actualDuration);
    this.userPatterns.schedulingPatterns.schedulingAccuracy =
      (this.userPatterns.schedulingPatterns.schedulingAccuracy * 9 + accuracy) / 10;
  }
}
