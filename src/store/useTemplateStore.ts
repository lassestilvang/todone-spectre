...
import { Template as TemplateType, TemplateCategory as TemplateCategoryType } from '../types/template';
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        addTemplate: (templateData: Omit<TemplateType, 'id' | 'createdAt' | 'updatedAt'>) => {
        ...
        updateTemplate: (id: string, updates: Partial<Omit<TemplateType, 'id' | 'createdAt'>>) => {
          set((state) => ({
        ...
        addCategory: (categoryData: Omit<TemplateCategoryType, 'id' | 'createdAt' | 'updatedAt'>) => {
        ...
        updateCategory: (id: string, updates: Partial<Omit<TemplateCategoryType, 'id' | 'createdAt'>>) => {
          set((state) => ({
        ...
        setTemplateSort: (sortBy: TemplateState['sortBy'], sortDirection: TemplateState['sortDirection']) => {
        ...
        applyTemplateFilters: () => {
          ...
          if (currentFilter.categoryId) {
            filtered = filtered.filter((template) => template.categoryId === currentFilter.categoryId);
          }
          ...
          if (currentFilter.isPublic !== undefined) {
            filtered = filtered.filter((template) => template.isPublic === currentFilter.isPublic);
          }
        ...
        bulkDeleteTemplates: (templateIds: string[]) => {
          set((state) => ({
            templates: state.templates.filter((template) => !templateIds.includes(template.id)),
          }));
          get().applyTemplateFilters();
        },

        // Preview operations
        setPreviewTemplate: (templateId: string, content: string, variables: Record<string, string>) => {
          set({
            previewTemplate: {
              templateId,
              content,
          ...
          const sampleTemplates: TemplateType[] = [
            {
              ...
              variables: {
                projectName: 'Todone',
                projectDescription: 'A comprehensive task management application',
