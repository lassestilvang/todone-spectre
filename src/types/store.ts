...
export interface CalendarState {
  ...
  // Event operations
  addEvent: (eventData: Omit<CalendarEventType, 'id'>) => Promise<CalendarEventType>;
  updateEvent: (eventId: string, updates: Partial<CalendarEventType>) => Promise<CalendarEventType | null>;
  deleteEvent: (eventId: string) => Promise<boolean>;
  fetchEvents: () => Promise<CalendarEventType[]>;

  // Calendar operations
  addCalendar: (calendarData: Omit<CalendarType, 'id'>) => Promise<CalendarType>;
  fetchCalendars: () => Promise<CalendarType[]>;
  updateCalendarConfig: (updates: Partial<CalendarConfig>) => void;

  // Sync operations
  syncCalendars: (calendarIds: string[]) => Promise<{ success: boolean; syncedEvents: CalendarEventType[] }>;
  getSyncStatus: () => { status: CalendarSyncStatus; lastSynced?: Date; error?: Error | null };
...
export interface CollaborationState {
  ...
  updateMember: (memberId: string, updates: Partial<CollaborationMember>) => void;
  deleteMember: (memberId: string) => void;

  addActivity: (activityData: Omit<CollaborationActivity, 'id'>) => void;
  updateActivity: (activityId: string, updates: Partial<CollaborationActivity>) => void;
  deleteActivity: (activityId: string) => void;

  // Filtering and Sorting
  setFilter: (filter: CollaborationState['currentFilter']) => void;
  setSort: (sortBy: CollaborationState['sortBy'], sortDirection: CollaborationState['sortDirection']) => void;
  applyFilters: () => void;

  // Settings operations
  updateSettings: (teamId: string, settings: Partial<CollaborationSettings>) => void;
...
export interface AIState {
  ...
  aiTaskBreakdowns: Record<string, {
    steps: string[];
    estimatedTime: string;
    dependencies: string[];
    resources: string[];
  }>;
  aiActionableItems: Record<string, {
    id: string;
    title: string;
    description: string;
