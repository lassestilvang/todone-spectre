...
export const useTaskHierarchy = () => {
  ...
  const getSubTasks = useCallback(
    async (parentTaskId: string): Promise<Task[]> => {
      try {
        return await taskHierarchyService.getSubTasks(parentTaskId);
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Failed to get sub-tasks",
        );
        return [];
      }
    },
   ...
   */
  const getTaskHierarchyTree = useCallback(
    async (parentTaskId: string): Promise<Task[]> => {
      try {
        setIsLoading(true);
        const tree =
          await taskHierarchyService.getTaskHierarchyTree(parentTaskId);
        setHierarchyTree(tree);
        setIsLoading(false);
        return tree;
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Failed to get hierarchy tree",
        );
        setIsLoading(false);
        return [];
      }
   ...
   * Get flat hierarchy
   */
  const getFlatHierarchy = useCallback(
    async (parentTaskId: string): Promise<Task[]> => {
      try {
        setIsLoading(true);
        ...
        setError(
          err instanceof Error ? err.message : "Failed to get flat hierarchy",
        );
        setIsLoading(false);
        return [];
      }
  ...
  const createSubTask = useCallback(
    async (
      ...
      try {
        setError(null);
        const newSubTask = await taskHierarchyService.createSubTaskInHierarchy(
          parentTaskId,
          subTaskData,
        );
   ...
   */
  const updateSubTaskPosition = useCallback(
    async (
      subTaskId: string,
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to update sub-task position",
        );
        throw err;
      }
    },
  ...
  const moveSubTask = useCallback(
    async (subTaskId: string, newParentTaskId: string): Promise<Task> => {
      try {
        setError(null);
        const movedSubTask = await taskHierarchyService.moveSubTask(
          subTaskId,
          newParentTaskId,
        );
        ...
        setError(
          err instanceof Error ? err.message : "Failed to move sub-task",
        );
        throw err;
      }
    },
   ...
   */
  const calculateHierarchyCompletion = useCallback(
    async (taskId: string): Promise<number> => {
      try {
        const percentage =
          await taskHierarchyService.calculateHierarchyCompletion(taskId);
        setCompletionPercentage(percentage);
        return percentage;
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Failed to calculate completion",
        );
        return 0;
      }
    },
   ...
   */
  const getTasksByLevel = useCallback(
    async (level: number): Promise<Task[]> => {
      try {
        return await taskHierarchyService.getTasksByLevel(level);
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Failed to get tasks by level",
        );
        return [];
      }
    },
  ...
  const findRootParent = useCallback(
    async (taskId: string): Promise<Task | null> => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to find root parent",
        );
        return null;
      }
    },
   ...
   */
  const refreshAffectedHierarchies = useCallback(
    async (subTaskId: string, newParentTaskId: string) => {
      try {
