...
export const useNlpParser = (options: UseNlpParserOptions = {}) => {
  ...
  const parseWithContext = useCallback(async (text: string, context: any = {}) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      const result = await parserService.parseWithContext(text, mergedContext);
      setLastResult(result);
      setParseHistory(prev => [...prev, result]);
  ...
  const parseBatch = useCallback(async (texts: string[]) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      const error = err instanceof Error ? err : new Error('Unknown batch parsing error');
      setError(error);
      throw error;
    } finally {
      setIsLoading(false);
