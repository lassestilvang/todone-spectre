...
export const useOffline = () => {
  ...
  const [status, setStatus] = useState<OfflineStatus>(isOffline ? 'offline' : 'online');
  ...
  }, [isOffline, status, pendingChanges, queue.length, lastSync, error, isProcessing]);
  ...
  const enqueueOperation = useCallback(async (
    ...
    try {
      ...
      if (!shouldQueue) {
        return {
          success: false,
          error: new Error('Cannot enqueue operation when online (except for sync operations)')
        };
      }
      ...
      if (queue.length >= settings.maxQueueSize) {
        return {
          success: false,
          error: new Error(`Queue full. Maximum size: ${settings.maxQueueSize}`)
        };
      }
      ...
      return {
        success: false,
        error: err instanceof Error ? err : new Error('Failed to enqueue operation')
      };
    }
  ...
  const processOfflineQueue = useCallback(async (): Promise<{
    ...
    try {
      ...
      const processedItems = queue.filter(item => item.status === 'completed');
      ...
      return {
        success: false,
        error: err instanceof Error ? err : new Error('Queue processing failed')
      };
    }
   ...
   */
  const getQueueItemsByStatus = useCallback((statusFilter: OfflineQueueItem['status']): OfflineQueueItem[] => {
