...
export const useAITaskSuggestions = (): AITaskSuggestionsState & {
  generateSuggestions: (taskId: string, options?: GenerateSuggestionsOptions) => Promise<void>;
  ...
  const generateSuggestions = useCallback(async (taskId: string, options?: GenerateSuggestionsOptions) => {
    try {
      setLoading(true);
      setError(null);
      setSuggestions([]);
  ...
  const generateSuggestionStats = useCallback((taskSuggestions: any[], taskId: string) => {
    ...
    const confidenceScores = taskSuggestions.map((_, index) => {
      ...
      if (task) {
        ...
        if (suggestion.includes(task.title) || (task.description && suggestion.includes(task.description))) {
          confidence += 20;
        }
      }
    ...
    const estimatedTimes = taskSuggestions.map(suggestion => {
      ...
      if (text.length > 80 || text.includes('research') || text.includes('complex')) {
        return '1-2 hours';
  ...
  const getSuggestionStats = useCallback(async (taskId: string) => {
    if (!taskId) {
      throw new Error('Task ID cannot be empty');
    }

    try {
      const taskSuggestions = await aiTaskService.getTaskSuggestions(taskId, 5);
