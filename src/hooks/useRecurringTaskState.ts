...
export interface UseRecurringTaskStateReturn {
  ...
  // Pattern Management
  updateRecurringPatternWithState: (taskId: string, pattern: RecurringPattern) => Promise<void>;
  updateRecurringConfigWithState: (taskId: string, config: RecurringTaskConfig) => Promise<void>;
  ...
  bulkUpdateRecurringTaskStatusWithState: (taskIds: string[], status: TaskStatus) => Promise<void>;
...
export const useRecurringTaskState = (): UseRecurringTaskStateReturn => {
   ...
   */
  const createRecurringTaskWithState = useCallback(async (
    taskData: Omit<Task, 'id' | 'createdAt' | 'updatedAt' | 'completed'>,
    config: RecurringTaskConfig
  ): Promise<Task> => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to create recurring task with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const updateRecurringTaskWithState = useCallback(async (
    taskId: string,
    updates: Partial<Task>,
    configUpdates?: Partial<RecurringTaskConfig>
  ): Promise<Task> => {
    try {
      ...
      const updatedTask = await updateRecurringTaskOriginal(taskId, updates, configUpdates);
      ...
      setError(err instanceof Error ? err.message : 'Failed to update recurring task with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const deleteRecurringTaskWithState = useCallback(async (taskId: string): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete recurring task with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const completeRecurringInstanceWithState = useCallback(async (instanceId: string): Promise<Task> => {
    try {
      setLoading(true);
      setError(null);

      // Complete using original service
      const completedInstance = await completeRecurringInstanceOriginal(instanceId);
      ...
      setError(err instanceof Error ? err.message : 'Failed to complete recurring instance with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const generateNextInstanceWithState = useCallback(async (taskId: string): Promise<Task | null> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to generate next instance with state');
      throw err;
    } finally {
      setLoading(false);
    }
  }, [
    generateNextInstanceOriginal,
    addRecurringInstance,
    setLoading,
    setError
  ]);
   ...
   */
  const regenerateAllInstancesWithState = useCallback(async (taskId: string): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to regenerate instances with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const updateRecurringPatternWithState = useCallback(async (taskId: string, pattern: RecurringPattern): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to update recurring pattern with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const updateRecurringConfigWithState = useCallback(async (taskId: string, config: RecurringTaskConfig): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to update recurring config with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const pauseRecurringTaskWithState = useCallback(async (taskId: string): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      // Update task status
      await updateRecurringTaskWithState(taskId, { status: 'archived' as TaskStatus });

    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to pause recurring task with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const resumeRecurringTaskWithState = useCallback(async (taskId: string): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      // Update task status
      await updateRecurringTaskWithState(taskId, { status: 'active' as TaskStatus });

    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to resume recurring task with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const bulkDeleteRecurringTasksWithState = useCallback(async (taskIds: string[]): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to bulk delete recurring tasks with state');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const bulkUpdateRecurringTaskStatusWithState = useCallback(async (taskIds: string[], status: TaskStatus): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError(err instanceof Error ? err.message : 'Failed to bulk update recurring task status with state');
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  const syncRecurringTasksWithStore = useCallback(async (): Promise<void> => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to sync recurring tasks with store');
      throw err;
    } finally {
      setLoading(false);
    }
   ...
   */
  const getRecurringTaskStatsWithState = useCallback((taskId: string) => {
