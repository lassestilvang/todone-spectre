...
describe('useCollaboration Hook', () => {
  ...
  it('should handle errors when creating a team', async () => {
    ...
    (collaborationService.createTeam as jest.Mock).mockRejectedValue(new Error(errorMessage));
...
describe('useCollaborationActivity Hook', () => {
  beforeEach(() => {
    ...
    // Mock the service methods
    (collaborationActivityService.getActivitiesByTeam as jest.Mock).mockResolvedValue([]);
    (collaborationActivityService.createActivity as jest.Mock).mockResolvedValue({
      id: 'test-activity',
      teamId: 'team-1',
      userId: 'user-1',
      action: 'Test action',
  ...
  it('should fetch activities when teamId is provided', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useCollaborationActivity('team-1'));

    // Wait for the initial fetch to complete
    await waitForNextUpdate();

    expect(collaborationActivityService.getActivitiesByTeam).toHaveBeenCalledWith('team-1');
    expect(result.current.loading).toBe(false);
  });
  ...
  it('should filter activities by type', () => {
    const mockActivities = [
      { id: '1', type: 'message', action: 'Test message', teamId: 'team-1', userId: 'user-1', timestamp: new Date() },
      { id: '2', type: 'task', action: 'Test task', teamId: 'team-1', userId: 'user-1', timestamp: new Date() }
    ];
