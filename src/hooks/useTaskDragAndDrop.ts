...
interface TaskDragAndDropOptions {
  ...
  onTaskDrop?: (taskId: string, targetId: string, position?: 'before' | 'after') => void;
}
...
export const useTaskDragAndDrop = (options: TaskDragAndDropOptions = {}) => {
  ...
  const { reorderTask, moveTask, moveTaskToProject, moveTaskToColumn } = useTaskStore();

  const handleTaskDragStart = useCallback((task: Task, source: string = 'task-list') => {
    setDraggedTask(task);
    setIsDragging(true);
  ...
  const handleTaskDrop = useCallback(async (targetId: string, position: 'before' | 'after' = 'after') => {
    ...
    try {
      ...
      } else if (dragSource === 'project' && targetId.startsWith('project-')) {
  ...
  }, [draggedTask, dragSource, reorderTask, moveTaskToColumn, moveTaskToProject, options.onTaskDrop, handleTaskDragEnd]);

  const handleTaskMove = useCallback(async (taskId: string, targetTaskId: string, position: 'before' | 'after' = 'after') => {
    try {
