...
export const useProjects = () => {
   ...
   */
  const createProject = useCallback(async (projectData: Omit<Project, 'id'>): Promise<Project> => {
    try {
      setLoading(true);
      setError(null);

      const response: ApiResponse<Project> = await projectApi.createProject(projectData);
  ...
  const updateProject = useCallback(async (projectId: string, updates: Partial<Project>): Promise<Project> => {
    try {
      ...
      const response: ApiResponse<Project> = await projectApi.updateProject(projectId, updates);
  ...
  const deleteProject = useCallback(async (projectId: string): Promise<void> => {
    try {
      ...
      const response: ApiResponse<void> = await projectApi.deleteProject(projectId);
   ...
   */
  const getProjectsByTeam = useCallback((teamId: string): Project[] => {
