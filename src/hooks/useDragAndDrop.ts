...
export const useDragAndDrop = (options: UseDragAndDropOptions = {}) => {
  ...
  const handleDragStart = useCallback((task: Task, source: string = 'task-list') => {
    setDraggedTask(task);
    setIsDragging(true);
  ...
  const handleDrop = useCallback(async (targetId: string, position: 'before' | 'after' = 'after') => {
  ...
  }, [draggedTask, dragSource, reorderTask, moveTask, options.onDrop, handleDragEnd]);
