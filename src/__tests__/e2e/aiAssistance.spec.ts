...
test.describe("AI Assistance Flow", () => {
  test("AI assistance: Create task with natural language → Get suggestions → Apply suggestions", async ({
    page,
  }) => {
    ...
    await test.step("Create task using natural language", async () => {
      ...
      await page.fill(
        'textarea[name="naturalLanguageInput"]',
      ...
      await expect(
        page.locator("text=Task suggestions generated"),
      ).toBeVisible();
    });
    ...
    await test.step("Review and apply AI suggestions", async () => {
      ...
      await expect(
        page.locator("text=Task created with AI assistance"),
      ).toBeVisible();

      // Verify task was created with AI suggestions
      await expect(
        page.locator("text=Website Redesign Project Plan"),
      ).toBeVisible();
      await expect(
        page.locator("text=Comprehensive project plan"),
      ).toBeVisible();
    });
    ...
    await test.step("Get additional AI suggestions", async () => {
      ...
      await expect(
        page.locator("text=Generating additional suggestions"),
      ).toBeVisible();
      ...
      await expect(
        page.locator("text=Suggestion applied successfully"),
      ).toBeVisible();
    });
  });
  ...
  test("AI assistance error recovery", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Please enter a description for AI to analyze"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Please provide more details for better suggestions"),
    ).toBeVisible();
  ...
  test("AI task breakdown and prioritization", async ({ page }) => {
    ...
    await page.fill(
      'textarea[name="complexTaskInput"]',
    ...
    await expect(
      page.locator("text=Subtasks created successfully"),
    ).toBeVisible();
  });
});
...
test.describe("AI Performance and Integration", () => {
  test("AI assistance with different task types", async ({ page }) => {
    ...
    const testCases = [
      {
        input:
          "Schedule a team meeting for project kickoff next Monday at 10 AM",
