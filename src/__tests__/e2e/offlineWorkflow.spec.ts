...
import { TEST_TASK, generateRandomTask } from './utils/testData';

test.describe('Offline Workflow', () => {
  test('Offline workflow: Go offline → Create tasks → Reconnect → Verify sync', async ({ page, context }) => {
    ...
    await test.step('Create tasks while offline', async () => {
      const offlineTasks = [generateRandomTask(), generateRandomTask(), generateRandomTask()];
    ...
    await test.step('Reconnect and verify sync', async () => {
      ...
      await expect(page.locator('text=All tasks synced successfully')).toBeVisible();
  ...
  test('Offline error recovery scenarios', async ({ page }) => {
    ...
    await expect(page.locator('text=Task validation failed - will not be queued')).toBeVisible();
  ...
  test('Offline task editing and conflict resolution', async ({ page }) => {
    ...
    await page.fill('textarea[name="description"]', 'Updated description while offline');
    await page.click('button[type="submit"]:has-text("Update")');

    await expect(page.locator('text=Task update queued for sync')).toBeVisible();
    ...
    await page.reload();
    await page.click(`text="${TEST_TASK.title}"`);
    await expect(page.locator('text=Updated description while offline')).toBeVisible();
  });
});

test.describe('Offline Collaboration Scenarios', () => {
  test('Offline team collaboration with sync conflicts', async ({ page, context }) => {
    await login(page, TEST_USER);
    ...
    await page.reload();
    await page.click('text=Collaboration Test Task');
    await expect(page.locator(`text=Assigned to ${TEST_USER.name}`)).toBeVisible();
  });
});