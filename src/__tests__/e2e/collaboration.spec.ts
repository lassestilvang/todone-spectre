...
test.describe('Collaboration Flow', () => {
  test('Complete collaboration workflow: Create team → Invite members → Assign tasks → Real-time updates', async ({ page, context }) => {
    // Step 1: Create team
    ...
    await test.step('Invite team member', async () => {
      ...
      await expect(page.locator(`text=Invitation sent to ${TEST_TEAM_MEMBER.email}`)).toBeVisible();
    });
    ...
    await test.step('Team member accepts invitation', async () => {
      ...
      await expect(memberPage.locator(`text=Team invitation from ${TEST_USER.name}`)).toBeVisible();
      await memberPage.click('button:has-text("Accept Invitation")');

      await expect(memberPage.locator(`text=Successfully joined ${TEST_TEAM.name}`)).toBeVisible();
    ...
    await test.step('Assign task to team member', async () => {
      ...
      await page.selectOption('select[name="assignee"]', TEST_TEAM_MEMBER.email);
      await page.click('button:has-text("Assign Task")');

      await expect(page.locator(`text=Assigned to ${TEST_TEAM_MEMBER.name}`)).toBeVisible();
    });
    ...
    await test.step('Verify real-time updates', async () => {
      ...
      await expect(memberPage.locator(`text="${TEST_TASK.title}"`)).toBeVisible();
      await expect(memberPage.locator(`text=Assigned to you`)).toBeVisible();

      // Team leader adds comment
      await page.click(`text="${TEST_TASK.title}"`);
      await page.click('button:has-text("Add Comment")');
      await page.fill('textarea[name="comment"]', 'Important update for this task');
      await page.click('button[type="submit"]:has-text("Post Comment")');

      // Verify team member sees comment in real-time
      await memberPage.reload();
      await expect(memberPage.locator('text=Important update for this task')).toBeVisible();
  ...
  test('Collaboration error recovery scenarios', async ({ page }) => {
    ...
    await page.click('button:has-text("Assign")');
    await page.selectOption('select[name="assignee"]', 'nonexistent@example.com');
    await page.click('button:has-text("Assign Task")');
