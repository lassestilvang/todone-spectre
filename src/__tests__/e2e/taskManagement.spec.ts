...
test.describe("Task Management Flow", () => {
  ...
  test("Complete task management workflow: Create → Set priority → Add comments → Complete task", async ({
    page,
  }) => {
    // Step 1: Create task
    ...
    await test.step("Complete task", async () => {
      ...
      await expect(
        page.locator("text=Task completed successfully"),
      ).toBeVisible();
    });
  });

  test("Task management with multiple tasks and filtering", async ({
    page,
  }) => {
...
test.describe("Task Management Edge Cases", () => {
  ...
  test("Task with very long content", async ({ page }) => {
    ...
    await expect(
      page.locator(`text="${longTitle.substring(0, 50)}..."`),
    ).toBeVisible();
  });
