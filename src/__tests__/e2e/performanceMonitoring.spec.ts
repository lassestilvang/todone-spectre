...
test.describe('Performance Monitoring', () => {
  test('Performance monitoring: Complete complex workflows → Check performance metrics', async ({ page }) => {
    ...
    await test.step('Check performance metrics', async () => {
      ...
      const apiTime = await page.locator('text=API Response Time').textContent();
...
test.describe('Performance Edge Cases', () => {
  test('Performance with rapid user interactions', async ({ page }) => {
    ...
    await test.step('Test rapid task creation', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await page.fill('textarea[name="description"]', `Description for rapid task ${i}`);
        await page.click('button[type="submit"]:has-text("Create")');
      ...
      console.log(`Created 5 tasks in ${totalTime}ms (avg ${avgTimePerTask}ms per task)`);
    ...
    await test.step('Test rapid filtering and sorting', async () => {
      ...
      console.log(`Performed ${operations.length} filter/sort operations in ${totalTime}ms (avg ${avgTimePerOp}ms per operation)`);
  ...
  test('Performance with network throttling', async ({ page }) => {
    ...
    await test.step('Simulate slow network', async () => {
      ...
      await page.fill('textarea[name="description"]', 'Testing network conditions');
      await page.click('button[type="submit"]:has-text("Create")');
