...
test.describe('API Performance Tests', () => {
  ...
  test('Task operations API performance', async ({ page }) => {
    ...
    await test.step('Test bulk task operations performance', async () => {
      ...
      console.log(`Bulk task creation (10 tasks) took ${bulkTime.toFixed(2)}ms total, ${avgTimePerTask.toFixed(2)}ms avg per task`);
  ...
  test('Collaboration updates API performance', async ({ page }) => {
    ...
    await test.step('Test real-time collaboration updates', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await page.fill('textarea[name="comment"]', `Collaboration update ${i}`);
      ...
      console.log(`Average collaboration update time: ${avgUpdateTime.toFixed(2)}ms`);
    ...
    await test.step('Test concurrent collaboration performance', async () => {
      ...
      console.log(`Concurrent collaboration updates (6 total) took ${concurrentTime.toFixed(2)}ms`);
  ...
  test('API response time analysis', async ({ page }) => {
    ...
    await test.step('Analyze API response times', async () => {
      ...
      for (const endpoint of endpoints) {
        ...
        console.log(`${endpoint.method} ${endpoint.name}: ${responseTime.toFixed(2)}ms`);
      }
      ...
      const avgResponseTime = responseTimes.reduce((sum, result) => sum + result.time, 0) / responseTimes.length;
