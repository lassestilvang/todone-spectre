...
test.describe('Core Workflow Performance Tests', () => {
  test('Task creation performance with varying dataset sizes', async ({ page }) => {
    ...
    for (const size of datasetSizes) {
      await test.step(`Test task creation with ${size} tasks`, async () => {
        ...
        console.log(`Created ${size} tasks: ${totalTime.toFixed(2)}ms total, ${avgTimePerTask.toFixed(2)}ms avg per task`);
  ...
  test('Project loading performance with complex structures', async ({ page }) => {
    await login(page, TEST_USER);
    ...
    await test.step('Create complex project structure', async () => {
      ...
      console.log(`Created complex project structure in ${totalTime.toFixed(2)}ms`);
  ...
  test('Search functionality performance with large datasets', async ({ page }) => {
    await login(page, TEST_USER);
    ...
    await test.step('Create large dataset for search testing', async () => {
      for (let i = 0; i < 50; i++) {
        ...
        await page.click('button:has-text("Create Task")');
        await page.fill('input[name="title"]', `${task.title} ${i}`);
        await page.fill('textarea[name="description"]', `Searchable content ${i} ${task.description}`);
        await page.selectOption('select[name="priority"]', task.priority);
        await page.click('button[type="submit"]:has-text("Create")');
      }
    });
