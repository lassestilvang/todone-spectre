...
test.describe('Lighthouse Performance Audits', () => {
  ...
  test('Lighthouse audit with different network conditions', async ({ page }) => {
    await login(page, TEST_USER);
    ...
    await test.step('Test Lighthouse audits with network conditions', async () => {
      ...
      const slow3GResult = conditionResults.find(r => r.networkCondition === 'Slow 3G');
      if (slow3GResult) {
  ...
  test('Lighthouse audit for critical user journeys', async ({ page }) => {
    ...
    await test.step('Audit critical user journeys', async () => {
      ...
      for (const journey of userJourneys) {
        ...
        console.log(`${journey.name} - Time: ${journeyTime.toFixed(2)}ms, Performance: ${auditResult.performanceScore}`);
      }
