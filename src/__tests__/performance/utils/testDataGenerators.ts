...
export function generatePerformanceTestData(size: 'small' | 'medium' | 'large' | number) {
  const sizes = {
    small: 10,
    medium: 50,
    large: 200,
...
export function generatePerformanceProject(index: number) {
  return {
    ...
    status: faker.helpers.arrayElement(['active', 'completed', 'archived', 'on-hold']),
    priority: faker.helpers.arrayElement(['low', 'medium', 'high', 'critical']),
    createdAt: faker.date.past().toISOString(),
    dueDate: faker.date.future().toISOString(),
    teamSize: faker.datatype.number({ min: 1, max: 20 }),
    complexity: faker.helpers.arrayElement(['simple', 'moderate', 'complex', 'enterprise']),
...
export function generatePerformanceTask(index: number, users: any[] = [], projects: any[] = []) {
  const taskTypes = ['feature', 'bug', 'improvement', 'documentation', 'research', 'maintenance'];
  ...
  return {
    ...
    actualHours: faker.datatype.boolean() ? faker.datatype.number({ min: 0, max: 40 }) : null,
    assignee: users.length > 0 ? faker.helpers.arrayElement(users).id : null,
    project: projects.length > 0 ? faker.helpers.arrayElement(projects).id : null,
    dependencies: Array.from({ length: faker.datatype.number({ min: 0, max: 3 }) }, (_, i) =>
      `task-${index - i - 1}`
    ).filter(dep => dep !== `task-${index}`),
    tags: Array.from({ length: faker.datatype.number({ min: 1, max: 4 }) }, () =>
      faker.commerce.productAdjective()
    ),
    ...
    comments: Array.from({ length: faker.datatype.number({ min: 0, max: 3 }) }, () => ({
      id: `comment-${index}-${faker.string.uuid()}`,
      author: users.length > 0 ? faker.helpers.arrayElement(users).id : `user-${faker.datatype.number({ min: 0, max: 9 })}`,
      content: faker.lorem.sentences(faker.datatype.number({ min: 1, max: 2 })),
      createdAt: faker.date.recent().toISOString(),
      reactions: Array.from({ length: faker.datatype.number({ min: 0, max: 5 }) }, () =>
        faker.helpers.arrayElement(['👍', '❤️', '🚀', '🎉', '💡'])
      )
    })),
    history: Array.from({ length: faker.datatype.number({ min: 1, max: 5 }) }, (_, i) => ({
      id: `history-${index}-${i}`,
      action: faker.helpers.arrayElement([
        'created', 'updated', 'priority-changed', 'status-changed',
        'assigned', 'commented', 'attachment-added'
      ]),
      performedBy: users.length > 0 ? faker.helpers.arrayElement(users).id : `user-${faker.datatype.number({ min: 0, max: 9 })}`,
      ...
      details: {
        ...
        field: faker.helpers.arrayElement(['status', 'priority', 'assignee', 'description'])
      }
    }))
  };
}
