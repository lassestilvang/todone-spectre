...
describe('Calendar Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(CalendarService, 'createEvent').mockImplementation(async (eventData) => {
      const newEvent = {
        ...eventData,
        id: `event-${Date.now()}`,
        createdAt: new Date().toISOString(),
    ...
    vi.spyOn(CalendarService, 'getEvents').mockResolvedValue([mockCalendarEvent]);
    vi.spyOn(CalendarService, 'getEventById').mockResolvedValue(mockCalendarEvent);
  ...
  test('Task creation → Calendar sync → UI reflection flow', async () => {
    ...
    const linkResult = await CalendarService.linkTaskToEvent(mockTask.id, calendarEvent.id);
  ...
  test('Calendar event management and task synchronization', async () => {
    ...
    const updatedEvent = await CalendarService.updateEvent(createdEvents[0].id, {
  ...
  test('Calendar date range queries and task filtering', async () => {
    ...
    const futureEvents = await CalendarService.getEventsForDateRange(futureStart, futureEnd);
  ...
  test('Calendar and task integration scenarios', async () => {
    ...
    const allEvents = await CalendarService.getEvents();
    const taskRelatedEvents = allEvents.filter(event => event.taskId === mockTask.id);
