...
describe("AI Integration Tests", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(aiService, "generateTaskBreakdown").mockResolvedValue(
      mockTaskBreakdown,
    );
  ...
  test("AI integration with different task types", async () => {
    ...
    const complexTask = {
      ...
      description:
        "Implement OAuth 2.0 with multiple providers and security enhancements",
      priority: "high",
    };
    ...
    const complexComplexity =
      await aiService.analyzeTaskComplexity(complexTask);

    // Complex task should have higher complexity
    expect(complexComplexity.complexityScore).toBeGreaterThan(
      simpleComplexity.complexityScore,
    );
  });
  ...
  test("AI response parsing and data consistency", async () => {
    ...
    vi.spyOn(aiService, "generateAIResponse").mockResolvedValueOnce(
      mockResponse1,
    );
    vi.spyOn(aiService, "generateAIResponse").mockResolvedValueOnce(
      mockResponse2,
  ...
  test("AI integration with task management workflow", async () => {
    ...
    const taskDescription =
      "Create system that analyzes tasks and provides AI-generated suggestions";
    ...
    expect(breakdown.steps.some((s) => s.toLowerCase().includes("ai"))).toBe(
      true,
    );
  });
});
