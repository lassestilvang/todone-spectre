...
jest.mock('../../utils/auth', () => ({
  ...
  validateToken: jest.fn().mockReturnValue({ id: 'user-1', email: 'test@example.com' }),
  hashPassword: jest.fn().mockResolvedValue('hashed-password'),
  comparePasswords: jest.fn().mockResolvedValue(true),
  setSession: jest.fn(),
  clearSession: jest.fn(),
  getSession: jest.fn().mockReturnValue({ token: 'current-token', refreshToken: 'current-refresh-token' })
}));
...
describe('Auth API', () => {
  ...
  describe('register', () => {
    ...
    it('should throw AuthError for existing user', async () => {
      ...
      await expect(register('New User', 'existing@example.com', 'password123'))
        .rejects
        .toThrow(AuthError);
    });
  });
  ...
  describe('refreshToken', () => {
    ...
    it('should throw AuthorizationError when no refresh token available', async () => {
      ...
      require('../../utils/auth').getSession.mockReturnValueOnce({ token: null, refreshToken: null });
