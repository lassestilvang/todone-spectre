...
export async function runComprehensiveAccessibilityAudit(
  ...
  console.log(
    `📊 Testing ${components.length} components for WCAG 2.1 AA compliance`,
  );
  ...
  for (const { component, name } of components) {
    ...
    if (result.passed) {
      ...
      console.log(
        `❌ ${name}: FAILED (${result.violations.length} violations)`,
      );
  ...
  console.log(
    `🎯 Compliance: ${report.wcagCompliance.level} (${report.wcagCompliance.score}/100)`,
  );
