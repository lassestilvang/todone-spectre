...
import { generateMockTemplate, generateMockTemplateCategory, createMockTemplateService } from './templateTestUtils';
...
describe('TemplateList Component', () => {
  ...
  it('shows empty state when no templates', () => {
    ...
    expect(screen.getByText('No templates found. Create a new template to get started!')).toBeInTheDocument();
  });
});
...
describe('TemplateForm Component', () => {
  ...
  it('renders edit form with template data', () => {
    render(
      <TemplateProvider>
        <TemplateForm template={mockTemplate} onSave={mockOnSave} onCancel={() => {}} />
      </TemplateProvider>
    );
    ...
    expect(screen.getByLabelText('Template Content')).toHaveValue('Existing content with {{variable}}');
  });
  ...
  it('calls onSave when form is submitted with valid data', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Template Content'), { target: { value: 'New content' } });
...
describe('TemplatePreview Component', () => {
  ...
  it('renders template preview', () => {
    render(
      <TemplateProvider>
        <TemplatePreview template={mockTemplate} onClose={mockOnClose} onApply={mockOnApply} />
      </TemplateProvider>
    );
  ...
  it('switches between preview and variables tabs', () => {
    ...
    expect(screen.getByText('Customize the template variables before applying:')).toBeVisible();
  });
});