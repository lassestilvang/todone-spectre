...
interface ResponsiveMobileNavigationProps {
  ...
  onTabChange: (tab: 'tasks' | 'projects' | 'calendar' | 'settings' | 'search') => void;
  showLabels?: boolean;
  compactMode?: boolean;
}

export const ResponsiveMobileNavigation: React.FC<ResponsiveMobileNavigationProps> = ({
  activeTab,
  onTabChange,
  showLabels = true,
  compactMode = false,
  ...
  const handleTabPress = (tab: 'tasks' | 'projects' | 'calendar' | 'settings' | 'search') => {
    onTabChange(tab);
  ...
  const getTabIndicatorStyle = (isActive: boolean) => {
    if (isActive) {
      return {
        backgroundColor: mobileConfig.darkMode ? 'rgba(102, 187, 106, 0.2)' : 'rgba(76, 175, 80, 0.1)',
        borderTopWidth: 2,
    ...
    >
      {navItems.map((item) => (
        <TouchableOpacity
          ...
          onPress={() => handleTabPress(item.key as 'tasks' | 'projects' | 'calendar' | 'settings' | 'search')}
        >
          <View style={[styles.tabContent, navStyles.tabContent]}>
            <View style={[styles.tabIndicator, getTabIndicatorStyle(activeTab === item.key)]}>
              <Icon
                name={item.icon}
