...
describe('NlpService', () => {
  ...
  describe('parseNaturalLanguage', () => {
    it('should throw error for empty input', async () => {
      await expect(service.parseNaturalLanguage(''))
        .rejects
        .toThrow('Empty input text');
    });

    it('should throw error for invalid input', async () => {
      await expect(service.parseNaturalLanguage(null as any))
        .rejects
        .toThrow('Invalid input text');
    });

    it('should parse valid text and return structured data', async () => {
      const testText = 'Create a high priority task to finish the project by Friday';
...
describe('Integration Tests', () => {
  it('should integrate NlpService with NlpParserService', async () => {
    ...
    const testText = 'Create an important task for the marketing project due tomorrow';
