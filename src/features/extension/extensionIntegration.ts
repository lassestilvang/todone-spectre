...
class ExtensionIntegration {
   ...
   */
  private handleMessage(message: ExtensionMessage, sender: chrome.runtime.MessageSender): void {
  ...
  private handlePortConnection(port: chrome.runtime.Port): void {
    ...
    port.onMessage.addListener((message: ExtensionMessage) => {
      this.handleMessage(message, { id: port.name } as chrome.runtime.MessageSender);
    });
   ...
   */
  private async handleConfigUpdate(payload: Partial<ExtensionState>): Promise<void> {
    try {
   ...
   */
  public async sendMessageToContentScripts(message: ExtensionMessage): Promise<void> {
    try {
   ...
   */
  public async sendMessageToBackground(message: ExtensionMessage): Promise<any> {
    try {
    ...
    try {
      ...
      if (tab && tab.url) {
        const supportedDomains = [
          ...
          'google.com', 'outlook.com', 'office.com'
        ];
...
export const initializeExtensionIntegration = () => extensionIntegration.initialize();
...
export const getExtensionIntegrationState = () => extensionIntegration.getCurrentState();
export const getExtensionIntegrationConfig = () => extensionIntegration.getCurrentConfig();
export const isExtensionReady = () => extensionIntegration.isExtensionReady();
export const isPageIntegrationSupported = () => extensionIntegration.isPageIntegrationSupported();