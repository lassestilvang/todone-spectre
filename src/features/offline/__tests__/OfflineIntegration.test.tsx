...
describe("OfflineIntegration Component", () => {
  ...
  beforeEach(() => {
    ...
    mockUseOfflineTasks.mockReturnValue({
      ...
      createTaskOffline: jest
        .fn()
        .mockResolvedValue({ ...mockTask, id: "test-id" }),
  ...
  it("should render offline integration dashboard", () => {
    ...
    expect(
      screen.getByText("Comprehensive Offline Status"),
    ).toBeInTheDocument();
  });
  ...
  it("should handle offline task creation", async () => {
    const createTaskMock = jest
      .fn()
      .mockResolvedValue({ ...mockTask, id: "offline-id" });
    mockUseOfflineTasks.mockReturnValue({
      ...mockUseOfflineTasks(),
      isOffline: true,
  ...
  it("should display comprehensive offline status", () => {
    ...
    expect(
      screen.getByText(JSON.stringify(mockStatus, null, 2)),
    ).toBeInTheDocument();
  });
});
