...
export const MicroInteraction: React.FC<MicroInteractionProps> = ({
  ...
  const { microInteractionEnabled, triggerMicroInteraction, animationSpeed } =
    useAnimationContext();
  ...
  const getScaleForType = (type: string, intensity: number) => {
    switch (type) {
      case "click":
        return 0.95 * (2 - intensity);
      case "press":
        return 0.98 * (2 - intensity);
      case "tap":
        return 0.97 * (2 - intensity);
      case "hover":
        return 1.02 * intensity;
      case "focus":
        return 1.01 * intensity;
      case "drag":
        return 1.05 * intensity;
      default:
        return 1;
    }
  };
  ...
  const getOpacityForType = (type: string, intensity: number) => {
    switch (type) {
      case "click":
        return 0.9 * (2 - intensity);
      case "press":
        return 0.95 * (2 - intensity);
      case "tap":
        return 0.93 * (2 - intensity);
      default:
        return 1;
    }
  };
  ...
  return (
    <motion.div
      ...
      whileTap={
        type === "press" || type === "tap"
          ? { scale: getScaleForType(type, intensity) }
          : {}
      }
      whileHover={
        type === "hover" ? { scale: getScaleForType("hover", intensity) } : {}
      }
