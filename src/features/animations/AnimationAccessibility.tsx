...
export const AnimationAccessibility: React.FC<AnimationAccessibilityProps> = ({
  ...
  const { isAnimating, toggleAnimation, animationSpeed, setAnimationSpeed } = useAnimationContext();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.matchMedia('(prefers-contrast: more)').removeEventListener('change', checkSystemPreferences);
    };
  }, []);
  ...
  const applyAccessibilitySettings = () => {
    const shouldReduceMotion = reducedMotion || accessibilitySettings.prefersReducedMotion;
    const shouldUseHighContrast = highContrast || accessibilitySettings.prefersHighContrast;
  ...
  const getAccessibilityStatus = () => {
    ...
    return issues.length === 0 ? '✅ Accessible' : `⚠️ ${issues.length} issue(s)`;
  };
    ...
    >
      ...
      {(isAnimating || highContrast) && (
        <div style={{
          ...
          {accessibilitySettings.prefersReducedMotion && <span>🚫 Reduced Motion</span>}
          {accessibilitySettings.prefersHighContrast && <span>🎨 High Contrast</span>}
        </div>
      )}
    </div>
  );
