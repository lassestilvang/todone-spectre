...
const RecurringTaskPreview: React.FC<RecurringTaskPreviewProps> = ({
  ...
  const { getRecurringTaskStats, completeRecurringInstance } =
    useRecurringTasks();
            ...
            {stats.nextInstanceDate && (
              <div className="bg-purple-50 p-2 rounded-md">
                ...
                <div className="text-gray-800">
                  {format(new Date(stats.nextInstanceDate), "MMM d")}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex space-x-2">
            {previewInstances.length > 5 && (
              ...
              >
                {showAllInstances
                  ? "Show Less"
                  : `Show All (${previewInstances.length})`}
              </button>
            )}
            <button
              ...
              <div className="mt-3 p-2 bg-blue-50 border border-blue-100 rounded-md text-sm">
                <p className="text-blue-700">
                  ...
                  {task.customFields?.recurringCount && (
                    <span>
                      , {task.customFields.recurringCount} occurrences
                    </span>
                  )}
                  {task.customFields?.recurringEndDate && (
                    <span>
                      , ending{" "}
                      {format(
                        new Date(task.customFields.recurringEndDate),
                        "PPP",
                      )}
                    </span>
                  )}
                ...
                {task.customFields?.recurringConfig && (
                  <div className="mt-2 text-xs text-blue-600">
                    <div>
                      ...
                      {task.customFields.recurringConfig.customInterval || 1}
                    </div>
                    <div>
                      <strong>Unit:</strong>{" "}
                      {task.customFields.recurringConfig.customUnit || "weeks"}
                    </div>
                  </div>
                )}
              </div>
