...
import { mockRecurringTaskService, mockRecurringTaskIntegration } from '../../../utils/__tests__/recurringTaskServiceMocks';
...
describe('Recurring Task Component Tests', () => {
  ...
  beforeEach(() => {
    ...
    (useRecurringTasks as jest.Mock).mockReturnValue({
      createRecurringTask: jest.fn().mockImplementation((task, config) =>
        mockRecurringTaskService.createRecurringTask(task, config)
      ),
      fetchRecurringTasks: jest.fn().mockResolvedValue(mockTasks),
      completeRecurringInstance: jest.fn().mockImplementation((instanceId) =>
        mockRecurringTaskService.completeRecurringInstance(instanceId)
      ),
      getRecurringTaskStats: jest.fn().mockImplementation((taskId) =>
    ...
    (useRecurringTaskIntegration as jest.Mock).mockReturnValue({
      createRecurringTaskIntegrated: jest.fn().mockImplementation((task, config) =>
        mockRecurringTaskIntegration.createRecurringTaskIntegrated(task, config)
      ),
  ...
  describe('RecurringTaskForm Component', () => {
    ...
    test('should successfully create a recurring task with valid data', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Recurring task created successfully')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('RecurringTaskList Component', () => {
    test('should render list of recurring tasks', async () => {
      ...
      expect(screen.getByText('Loading recurring tasks...')).toBeInTheDocument();
    ...
    test('should show empty state when no recurring tasks exist', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('No recurring tasks found')).toBeInTheDocument();
        expect(screen.getByText('Create your first recurring task')).toBeInTheDocument();
      });
    });
    ...
    test('should allow filtering recurring tasks by pattern', async () => {
      ...
      expect(screen.getByText('Weekly Team Meeting')).toBeInTheDocument();
      expect(screen.queryByText('Daily Standup Meeting')).not.toBeInTheDocument();
    });
  ...
  describe('RecurringTaskPreview Component', () => {
    test('should render preview of recurring task with instances', async () => {
      ...
      expect(screen.getByText(`Instances: ${instances.length}`)).toBeInTheDocument();
    });
    ...
    test('should show statistics for recurring task', async () => {
      ...
      for (let i = 0; i < 2; i++) {
        await mockRecurringTaskService.completeRecurringInstance(instances[i].id);
      }
    ...
    test('should show timeline view for recurring task', async () => {
      ...
      expect(screen.getAllByText('Instance Generated').length).toBeGreaterThan(0);
    });
  });
  ...
  describe('RecurringTaskScheduler Component', () => {
    test('should render scheduler with upcoming instances', async () => {
      ...
      expect(screen.getAllByTestId('scheduler-instance').length).toBeGreaterThan(0);
    });
  ...
  describe('Integration Tests', () => {
    test('should integrate form, list, and preview components', async () => {
      ...
      const task = await mockRecurringTaskService.getRecurringTask(
        mockRecurringTaskService.getCallHistory()
    ...
    test('should handle error states gracefully', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Error loading recurring tasks')).toBeInTheDocument();
        expect(screen.getByText('Failed to load recurring tasks')).toBeInTheDocument();
      });
    });
    ...
    test('should show loading states appropriately', async () => {
      ...
      expect(screen.getByText('Loading recurring tasks...')).toBeInTheDocument();
      expect(screen.queryByText('Error loading recurring tasks')).not.toBeInTheDocument();
    });
  });
});