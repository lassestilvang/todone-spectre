...
const RecurringTaskList: React.FC<RecurringTaskListProps> = ({
  ...
  const filteredTasks = useMemo(() => {
    return tasks
      .filter((task) => {
        ...
        return (
          matchesSearch && matchesPattern && matchesStatus && matchesCompletion
        );
      })
      .sort((a, b) => {
        let comparison = 0;
  ...
  const getSortIndicator = (
    field: "date" | "title" | "priority" | "status",
  ) => {
                        ...
                        {stats && (
                          <>
                            ...
                            {stats.nextInstanceDate && (
                              <div>
                                <strong>Next Instance:</strong>{" "}
                                {format(
                                  new Date(stats.nextInstanceDate),
                                  "PPP",
                                )}
                              </div>
                            )}
