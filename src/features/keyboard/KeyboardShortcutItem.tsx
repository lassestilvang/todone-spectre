...
export const KeyboardShortcutItem: React.FC<KeyboardShortcutItemProps> = ({ shortcut }) => {
  ...
  return (
    <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
      <div className="flex-1">
        <div className="font-medium text-gray-900 dark:text-white">{shortcut.description}</div>
        {shortcut.category && (
          <div className="text-sm text-gray-500 dark:text-gray-400">{shortcut.category}</div>
        )}
      </div>
          ...
          {shortcut.modifiers && shortcut.modifiers.length > 0 && (
            <>
              {shortcut.modifiers.map((modifier, index) => (
                ...
                >
                  {modifier === 'meta' ? '⌘' : modifier === 'ctrl' ? 'Ctrl' : modifier}
                </kbd>
              ))}
