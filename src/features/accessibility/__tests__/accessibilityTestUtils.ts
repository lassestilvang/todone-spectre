import { AccessibilityFeature, AccessibilityState } from '../../../services/accessibilityService';
...
interface AccessibilityTestUtils {
  ...
  createMockAccessibilityState: (overrides?: Partial<AccessibilityState>) => AccessibilityState;
  createMockAccessibilityConfig: (overrides?: Partial<AccessibilityConfig>) => AccessibilityConfig;
  createMockAccessibilityFeature: (overrides?: Partial<AccessibilityFeature>) => AccessibilityFeature;
  generateTestScenarios: (count: number) => AccessibilityTestData;
  validateAccessibilityState: (state: AccessibilityState) => { isValid: boolean; errors: string[] };
  validateAccessibilityConfig: (config: AccessibilityConfig) => { isValid: boolean; errors: string[] };
  createAccessibilityTestContext: () => {
    state: AccessibilityState;
    config: AccessibilityConfig;
    features: AccessibilityFeature[];
  };
  generatePerformanceTestData: (size: 'small' | 'medium' | 'large') => AccessibilityTestData;
...
const accessibilityTestUtils: AccessibilityTestUtils = {
  generateTestData: (): AccessibilityTestData => {
    return {
      ...
      mockFeatures: [
        this.createMockAccessibilityFeature({ id: 'high-contrast', name: 'High Contrast', isEnabled: true }),
        this.createMockAccessibilityFeature({ id: 'reduce-motion', name: 'Reduce Motion', isEnabled: false }),
        this.createMockAccessibilityFeature({ id: 'screen-reader', name: 'Screen Reader', isEnabled: true })
      ],
      testScenarios: [
        { highContrast: true, fontSize: 'large', reduceMotion: false, screenReader: true, keyboardNavigation: true },
        { highContrast: false, fontSize: 'medium', reduceMotion: true, screenReader: false, keyboardNavigation: false },
        { highContrast: true, fontSize: 'xlarge', reduceMotion: true, screenReader: true, keyboardNavigation: true }
      ]
    };
  },
  ...
  generateTestScenarios: (count: number): AccessibilityTestData => {
    ...
    return {
      ...
      mockConfig: this.createMockAccessibilityConfig(),
      mockFeatures: [
        this.createMockAccessibilityFeature({ id: 'test-1', name: 'Test 1', isEnabled: true }),
        this.createMockAccessibilityFeature({ id: 'test-2', name: 'Test 2', isEnabled: false })
      ],
  ...
  validateAccessibilityConfig: (config: AccessibilityConfig): { isValid: boolean; errors: string[] } => {
    ...
    if (!validFontSizes.includes(config.defaultFontSize)) {
      errors.push(`defaultFontSize must be one of: ${validFontSizes.join(', ')}`);
    }
  ...
  createAccessibilityServiceMock: (): ReturnType<AccessibilityTestUtils['createAccessibilityServiceMock']> => {
    ...
    return {
      ...
      toggleHighContrast: () => {
        state = { ...state, isHighContrast: !state.isHighContrast, lastUpdated: new Date() };
      },
      setFontSize: (size: string) => {
        state = { ...state, fontSize: size, lastUpdated: new Date() };
      },
      toggleReduceMotion: () => {
        state = { ...state, reduceMotion: !state.reduceMotion, lastUpdated: new Date() };
      },
      toggleScreenReaderSupport: (enabled: boolean) => {
        state = { ...state, screenReaderEnabled: enabled, lastUpdated: new Date() };
      },
      toggleKeyboardNavigation: (enabled: boolean) => {
        state = { ...state, keyboardNavigation: enabled, lastUpdated: new Date() };
      },
      addFeature: (featureId: string) => {
        if (!state.features.includes(featureId)) {
          state = { ...state, features: [...state.features, featureId], lastUpdated: new Date() };
        }
      },
      removeFeature: (featureId: string) => {
        state = { ...state, features: state.features.filter(f => f !== featureId), lastUpdated: new Date() };
      },
      resetToDefaults: () => {
        state = this.createMockAccessibilityState();
        preferences = {
      ...
      getAccessibilityStatus: () => {
        ...
        if (score >= 80) {
          return { level: 'excellent', message: 'Excellent accessibility support', score };
        } else if (score >= 50) {
          return { level: 'good', message: 'Good accessibility support', score };
        } else if (score >= 20) {
          return { level: 'basic', message: 'Basic accessibility support', score };
        } else {
          return { level: 'minimal', message: 'Minimal accessibility support', score };
        }
      },
  ...
  createAccessibilityConfigServiceMock: (): ReturnType<AccessibilityTestUtils['createAccessibilityConfigServiceMock']> => {
    ...
    return {
      ...
      validateConfig: (configToValidate: Partial<AccessibilityConfig>) => {
        return this.validateAccessibilityConfig(configToValidate as AccessibilityConfig);
      },
      getConfigSummary: () => {
        const enabledFeatures = Object.values(config.featureDefaults).filter(v => v).length;
...
const calculateMockAccessibilityScore = (state: AccessibilityState): number => {
  let score = 0;
  if (state.isHighContrast) score += 20;
  if (state.reduceMotion) score += 15;
  if (state.screenReaderEnabled) score += 25;
  if (state.keyboardNavigation) score += 20;
  if (state.fontSize !== 'medium') score += 10;
  if (state.features.length > 0) score += 10;
