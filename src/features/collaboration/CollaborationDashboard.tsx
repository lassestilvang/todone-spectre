...
import { CollaborationTeam, CollaborationMember, CollaborationActivity } from '../../types/collaboration';
import { Users, Activity, Settings, Plus, Search, Filter, Clock, MessageSquare, FileText, Check, Crown, ShieldCheck, BarChart2, PieChart, LineChart, Calendar, TrendingUp, TrendingDown, AlertTriangle } from 'lucide-react';
...
export const CollaborationDashboard: React.FC<CollaborationDashboardProps> = ({
  ...
  const { activities, getActivityStats, filterActivitiesByType, filterActivitiesByDateRange, searchActivities } = useCollaborationActivity(teamId);
  ...
  useEffect(() => {
    ...
    const calculateMetrics = () => {
      try {
        ...
        const completionRate = Math.min(100, Math.max(0, (activityStats.total / (activityStats.total + 1)) * 100));
  ...
  }, [activities, activityFilter, filterActivitiesByType, filterActivitiesByDateRange, searchActivities]);
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'member': return <Users className="h-3 w-3 text-blue-500" />;
      case 'guest': return <ShieldCheck className="h-3 w-3 text-green-500" />;
      default: return null;
    }
  };
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Active: {teamMetrics.activeMembers}</p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{teamMetrics.totalActivities}</div>
                <p className="text-xs text-muted-foreground">Recent updates</p>
                <div className="flex space-x-1 mt-2">
                  {Object.entries(teamMetrics.activitiesByType).map(([type, count]) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <Button variant="outline" size="sm" className="w-full mt-2" onClick={onSettingsClick}>
                    Manage Settings
                  </Button>
                </div>
              </CardContent>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Team collaboration activity breakdown</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">Total Activities</span>
                    <span className="text-lg font-bold">{teamMetrics.totalActivities}</span>
                  </div>
                      ...
                      {Object.entries(teamMetrics.activitiesByType).map(([type, count]) => (
                        <div key={type} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Progress value={(count / teamMetrics.totalActivities) * 100} className="w-20 h-2" />
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-bold">{teamMetrics.totalMembers}</span>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="text-sm font-medium">{teamMetrics.adminCount}</span>
                          <Progress value={(teamMetrics.adminCount / teamMetrics.totalMembers) * 100} className="w-20 h-2" />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <Progress value={(teamMetrics.memberCount / teamMetrics.totalMembers) * 100} className="w-20 h-2" />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="text-sm font-medium">{teamMetrics.guestCount}</span>
                          <Progress value={(teamMetrics.guestCount / teamMetrics.totalMembers) * 100} className="w-20 h-2" />
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                {filteredActivities.length === 0 ? (
                  ...
                  filteredActivities.map((activity) => (
                    <div key={activity.id} className="flex items-start space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors border">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2">
                          <Badge variant="secondary" className={getActivityColor(activity.type)}>
                            ...
                            <span className="ml-1 text-xs capitalize">{activity.type.replace('_', ' ')}</span>
                          </Badge>
                      ...
                      <div key={member.id} className="flex items-center justify-between py-4">
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center space-x-2">
                              ...
                              <Badge variant={member.role === 'admin' ? 'default' : member.role === 'member' ? 'secondary' : 'outline'} className="text-xs">
                        ...
                        <div className="flex items-center space-x-2">
                          <Badge variant="outline" className="text-xs">
                            ...
                            {activities.filter(a => a.userId === member.userId).length} activities
                          </Badge>
                        </div>
                      </div>
                    ))}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <CardContent>
            ...
            <div className="flex items-center space-x-1 mt-2">
              ...
              <span className="text-xs">
                {teamMetrics.totalActivities > 10 ? 'Active team' : 'Getting started'}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={(teamMetrics.activeMembers / teamMetrics.totalMembers) * 100} className="mt-2 h-2" />
          </CardContent>
        </Card>
