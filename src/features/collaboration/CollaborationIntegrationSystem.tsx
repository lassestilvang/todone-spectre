...
export const CollaborationIntegrationSystem: React.FC<
  CollaborationIntegrationSystemProps
> = ({
  teamId,
  ...
  const teamTasks = tasks.filter((task) =>
    teamMembers.some((member) => member.userId === task.assigneeId),
  );
    ...
    if (currentTeam) {
      ...
      const completionRate =
        teamTasks.length > 0
          ? Math.min(
              100,
              (teamTasks.filter((t) => t.completed).length / teamTasks.length) *
                100,
            )
          : 0;
  ...
  const handleActivityCreated = async (activity: CollaborationActivity) => {
    try {
      ...
      setCollaborationActivities((prev) => [
        createdActivity,
        ...prev.slice(0, 19),
      ]);
    } catch (err) {
  ...
  const handleSettingsUpdated = async (
    updatedSettings: CollaborationSettings,
  ) => {
    try {
  ...
  const isLoading =
    collaborationLoading || activityLoading || tasksLoading || projectsLoading;
