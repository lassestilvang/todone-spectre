...
import { MockCollaborationService, createMockCollaborationServiceWithData } from './collaborationServiceMocks';
...
export class CollaborationTestingInfrastructure {
  ...
  generateCompleteTestEnvironment(): {
    ...
    const { team, members, settings, activities } = this.dataGenerators.generateCompleteTeam();
    const service = this.createMockServiceWithData({ team, members, settings, activities });
  ...
  private testDataGeneration(): boolean {
    try {
      ...
      if (!completeTeam.team || !completeTeam.members || completeTeam.members.length === 0) {
  ...
  private async testMockServices(): Promise<boolean> {
    try {
      ...
      const updatedTeam = await service.updateTeam(createdTeam.id, { name: 'Updated Team' });
      if (updatedTeam.name !== 'Updated Team') {
        throw new Error('Team update failed');
      }

      // Test member operations
      const testMember = this.dataGenerators.generateMockMember({ teamId: createdTeam.id });
      const createdMember = await service.addMemberToTeam(createdTeam.id, testMember);
      if (!createdMember.id) {
      ...
      const testActivity = this.dataGenerators.generateMockActivity({ teamId: createdTeam.id });
      ...
      const testSettings = this.dataGenerators.generateMockSettings({ teamId: createdTeam.id });
      const createdSettings = await service.updateTeamSettings(createdTeam.id, testSettings);
  ...
  private async testIntegrationScenarios(): Promise<boolean> {
    try {
      ...
      const { team, members, settings, activities, service } = this.generateCompleteTestEnvironment();
      ...
      const updatedSettings = await service.updateTeamSettings(newTeam.id, settings);
  ...
  async runPerformanceTests(): Promise<boolean> {
    ...
    try {
      ...
      console.log(`📊 Generated 100 complete teams in ${generationTime.toFixed(2)}ms`);
      ...
      for (let i = 0; i < 50; i++) {
        ...
        await service.addMemberToTeam(team.id, this.dataGenerators.generateMockMember({ teamId: team.id }));
        await service.createActivity(this.dataGenerators.generateMockActivity({ teamId: team.id }));
      }

      const serviceTime = performance.now() - testStartTime;
      console.log(`🔧 Performed 150 mock service operations in ${serviceTime.toFixed(2)}ms`);
  ...
  async runEdgeCaseTests(): Promise<boolean> {
    ...
    try {
      ...
      try {
        await service.removeMemberFromTeam('non-existent-team', 'non-existent-member');
        throw new Error('Error handling test failed - should have thrown');
      } catch (error) {
        // Expected error
      }
