...
export class CollaborationTestDataGenerators {
   ...
   */
  static generateMockTeam(
    overrides: Partial<CollaborationTeam> = {},
  ): CollaborationTeam {
    ...
    return {
      ...
      description:
        ...
        `Test team description for ${Math.floor(Math.random() * 1000)}`,
      ...
      members: overrides.members || [
        this.generateMockMember({
          teamId:
            overrides.id || `team-${Math.random().toString(36).substr(2, 9)}`,
          user: mockUser,
        }),
      ],
   ...
   */
  static generateMockMember(
    overrides: Partial<CollaborationMember> = {},
  ): CollaborationMember {
    const mockUser = this.generateMockUser();
    ...
    return {
      ...
      teamId:
        overrides.teamId || `team-${Math.random().toString(36).substr(2, 9)}`,
  ...
  static generateMockSettings(
    ...
    return {
      ...
      notificationSettings: {
        emailNotifications:
          overrides.notificationSettings?.emailNotifications || true,
        pushNotifications:
          overrides.notificationSettings?.pushNotifications || true,
        mentionNotifications:
          overrides.notificationSettings?.mentionNotifications || true,
        dailyDigest: overrides.notificationSettings?.dailyDigest || false,
        ...overrides.notificationSettings,
      },
      permissionSettings: {
        allowGuestInvites:
          overrides.permissionSettings?.allowGuestInvites || false,
        allowPublicSharing:
          overrides.permissionSettings?.allowPublicSharing || false,
        requireAdminApproval:
          overrides.permissionSettings?.requireAdminApproval || true,
        allowMemberInvites:
          overrides.permissionSettings?.allowMemberInvites || false,
      ...
      privacySettings: {
        ...
        allowExternalAccess:
          overrides.privacySettings?.allowExternalAccess || false,
        ...overrides.privacySettings,
      },
      integrationSettings: {
        calendarIntegration:
          overrides.integrationSettings?.calendarIntegration || false,
        taskIntegration:
          overrides.integrationSettings?.taskIntegration || false,
        fileIntegration:
          overrides.integrationSettings?.fileIntegration || false,
   ...
   */
  static generateMockActivity(
    overrides: Partial<CollaborationActivity> = {},
  ): CollaborationActivity {
    const mockUser = this.generateMockUser();
    ...
    return {
      ...
      action:
        overrides.action ||
        `Test activity action ${Math.floor(Math.random() * 1000)}`,
      type: overrides.type || "other",
      timestamp: overrides.timestamp || new Date(),
      details:
        overrides.details ||
        `Test activity details for ${Math.floor(Math.random() * 1000)}`,
      entityId:
        overrides.entityId ||
        `entity-${Math.random().toString(36).substr(2, 9)}`,
  ...
  static generateMockEvent(
    ...
    return {
      ...
      teamId:
        overrides.teamId || `team-${Math.random().toString(36).substr(2, 9)}`,
      userId:
        overrides.userId || `user-${Math.random().toString(36).substr(2, 9)}`,
      ...overrides,
    };
  }
  ...
  static generateCompleteTeam(teamId?: string): {
    ...
    const team = this.generateMockTeam({
      ...
      description:
        "A complete team with members, settings, and activities for testing",
      ownerId: owner.id,
      memberCount: 4,
      activityCount: 3,
    });
   ...
   */
  static generateMultipleMembers(
    count: number = 3,
    teamId?: string,
  ): CollaborationMember[] {
    const teamIdToUse =
   ...
   */
  static generateMultipleActivities(
    count: number = 5,
    teamId?: string,
  ): CollaborationActivity[] {
    const teamIdToUse =
      teamId || `team-${Math.random().toString(36).substr(2, 9)}`;
    const activityTypes: CollaborationActivity["type"][] = [
      "message",
      "file",
      "task",
      "member_added",
   ...
   */
  static generateTeamWithPrivacy(
   ...
   */
  static generateActivityWithType(
