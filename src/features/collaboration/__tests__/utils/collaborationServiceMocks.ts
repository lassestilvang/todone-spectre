...
export class MockCollaborationService {
   ...
   */
  async createTeam(teamData: Omit<CollaborationTeam, 'id'>): Promise<CollaborationTeam> {
  ...
  async removeMemberFromTeam(teamId: string, memberId: string): Promise<void> {
    const memberIndex = this.members.findIndex(m => m.id === memberId && m.teamId === teamId);
    if (memberIndex === -1) {
  ...
  async updateTeamSettings(teamId: string, settings: Partial<CollaborationSettings>): Promise<CollaborationSettings> {
    const existingSettingsIndex = this.settings.findIndex(s => s.teamId === teamId);

    const updatedSettings: CollaborationSettings = {
      ...(existingSettingsIndex !== -1 ? this.settings[existingSettingsIndex] : { teamId }),
      ...settings,
      updatedAt: new Date()
    };
   ...
   */
  async createActivity(activityData: Omit<CollaborationActivity, 'id'>): Promise<CollaborationActivity> {
