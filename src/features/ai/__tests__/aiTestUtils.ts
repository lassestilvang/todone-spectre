...
export class AITestUtils {
    ...
    return [
      {
        ...
        description:
          "Investigate available AI technologies and frameworks for task assistance",
        priority: "high",
      ...
      },
      {
        ...
        description:
          "Create comprehensive documentation of research and implementation",
        priority: "low",
        estimatedTime: "1 hour",
      },
    ].slice(0, count);
  ...
  static createAIServiceMock() {
    return {
      ...
      analyzeTaskComplexity: jest.fn().mockImplementation((task: any) => {
        return Promise.resolve({
          complexityScore: task.description?.length > 50 ? 75 : 40,
          complexityLevel: task.description?.length > 50 ? "high" : "medium",
        });
      }),
      generateTaskSuggestions: jest
        .fn()
        .mockImplementation((taskId: string, taskTitle: string) => {
          return Promise.resolve([
      ...
      generateTaskBreakdown: jest.fn().mockImplementation((task: any) => {
        return Promise.resolve({
  ...
  static createAITaskServiceMock() {
    return {
      analyzeTask: jest.fn().mockImplementation((task: any) => {
        return Promise.resolve({
          taskId: task.id,
          complexityScore: 65,
