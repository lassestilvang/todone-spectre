...
export const AINaturalLanguageTaskCreator: React.FC<
  AINaturalLanguageTaskCreatorProps
> = ({
  onTaskCreated,
  onCancel,
  ...
  const { parse, parseWithContext, isLoading, error, lastResult } =
    useNlpParser({
      debug: true,
  ...
  const generateNextSteps = (parsedData: ParsedTaskData): string[] => {
    ...
    if (parsedData.title.toLowerCase().includes("meeting")) {
      ...
      parsedData.title.toLowerCase().includes("develop") ||
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      alert(
        `Error parsing task: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setIsCreating(false);
    }
  ...
  return (
    <div className="ai-natural-language-task-creator">
      <div className="nlp-creator-header">
        ...
        <p>
          Create tasks using natural language - just describe what you need to
          do!
        </p>
      </div>
...
- 'Create marketing campaign for product launch with John and Sarah'"
              ...
              <label>
                <input
                  ...
                  onChange={(e) =>
                    setContextOptions({
                      ...contextOptions,
                      defaultProject: e.target.checked
                        ? "current-project"
                        : undefined,
                    })
                  }
      ...
      {parseResult && showPreview && (
        <div className="nlp-preview-section">
          ...
          <div className="preview-content">
            <div className="preview-field">
              ...
              <span className="field-value">
                {parseResult.title || "Untitled Task"}
              </span>
            </div>
            ...
            <div className="preview-field">
              ...
              <span className="field-value">
                <span
                  ...
                  style={{
                    backgroundColor: getPriorityColor(parseResult.priority),
                  }}
                >
          ...
          {parseResult.suggestedNextSteps &&
            parseResult.suggestedNextSteps.length > 0 && (
