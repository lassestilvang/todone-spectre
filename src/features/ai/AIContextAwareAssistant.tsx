...
export const AIContextAwareAssistant: React.FC<AIContextAwareAssistantProps> = ({
  taskId,
  projectId,
  mode = 'task',
  onContextSuggestion
  ...
  const [activeTab, setActiveTab] = useState<'analysis' | 'suggestions'>('suggestions');
  ...
  const analyzeTaskContext = async (taskId: string) => {
    try {
      ...
      const generatedSuggestions = await generateContextAwareSuggestions(analysis);
      setSuggestions(generatedSuggestions);

    } catch (error) {
      console.error('Context analysis error:', error);
      setAnalysisError(error instanceof Error ? error.message : 'Failed to analyze context');
  ...
  const analyzeProjectContext = async (projectId: string) => {
    try {
      ...
      setAnalysisError(error instanceof Error ? error.message : 'Failed to analyze project context');
      setContextAnalysis(null);
      setSuggestions([]);
    } finally {
      setIsAnalyzing(false);
  ...
  const analyzeGlobalContext = async () => {
    try {
      ...
      setAnalysisError(error instanceof Error ? error.message : 'Failed to analyze global context');
      setContextAnalysis(null);
      setSuggestions([]);
    } finally {
      setIsAnalyzing(false);
  ...
  const findRelatedTasks = (task: Task): Task[] => {
    ...
    return tasks.filter(t =>
      ...
      (t.title.toLowerCase().includes(task.title.toLowerCase()) ||
       (t.description && t.description.toLowerCase().includes(task.title.toLowerCase())) ||
       (task.description && t.description && t.description.toLowerCase().includes(task.description.toLowerCase())))
  ...
  const generateContextAwareSuggestions = async (analysis: ContextAnalysis): Promise<ContextAwareSuggestion[]> => {
  ...
  const generateTaskContextSuggestions = (analysis: ContextAnalysis): ContextAwareSuggestion[] => {
    ...
    if (task.description && task.description.length > 150) {
      suggestions.push({
        ...
        suggestion: 'This appears to be a complex task. Consider breaking it down into smaller subtasks.',
        contextType: 'task',
        priority: 'medium',
        confidence: 75,
        reasoning: 'Task description is quite long, indicating potential complexity',
    ...
    if (analysis.currentTaskContext.dependentTasks.length > 2) {
      suggestions.push({
        ...
        suggestion: 'This task is blocking multiple other tasks. Consider increasing its priority.',
        contextType: 'task',
        priority: 'high',
        confidence: 80,
  ...
  const generateProjectContextSuggestions = (analysis: ContextAnalysis): ContextAwareSuggestion[] => {
    ...
    if (project.priority === 'high' && analysis.projectContext.projectTasks.length > 5) {
      suggestions.push({
    ...
    if (analysis.projectContext.projectTasks.length > 10 &&
        analysis.projectContext.projectTasks.filter(t => t.status === 'pending').length > 5) {
      suggestions.push({
  ...
  const generateUserContextSuggestions = (analysis: ContextAnalysis): ContextAwareSuggestion[] => {
    const suggestions: ContextAwareSuggestion[] = [];
    const now = new Date();
    const hours = now.getHours();

    // Time-based productivity suggestion
    if (analysis.userContext.currentFocus === 'morning focus work' && hours >= 9 && hours < 12) {
      suggestions.push({
        id: `suggest-${Date.now()}-8`,
        suggestion: 'Morning is your peak productivity time. Focus on high-priority tasks now.',
        contextType: 'user',
        priority: 'medium',
        confidence: 80,
    ...
    if (analysis.userContext.productivityPattern === 'morning person' && hours >= 14) {
      suggestions.push({
        id: `suggest-${Date.now()}-9`,
        suggestion: 'Afternoon is typically lower productivity for you. Consider scheduling meetings or lighter tasks.',
        contextType: 'user',
        priority: 'low',
        confidence: 70,
    ...
    if (analysis.userContext.recentActivity.includes('planning')) {
      suggestions.push({
        ...
        suggestion: 'You were recently planning tasks. This might be a good time to execute some of those plans.',
        contextType: 'user',
        priority: 'medium',
        confidence: 65,
  ...
  const generateSystemContextSuggestions = (analysis: ContextAnalysis): ContextAwareSuggestion[] => {
    const suggestions: ContextAwareSuggestion[] = [];
    const now = new Date();
    const hours = now.getHours();
    ...
    if (analysis.systemContext.systemLoad === 'high') {
      suggestions.push({
        ...
        suggestion: 'System is currently under high load. Consider saving complex tasks for later.',
        contextType: 'system',
        priority: 'medium',
        confidence: 75,
    ...
    if (hours >= 17 || hours < 8) {
      suggestions.push({
        ...
        suggestion: 'Outside normal working hours. Consider if this task can wait until tomorrow.',
        contextType: 'system',
        priority: 'low',
        confidence: 60,
    ...
    // Weekend suggestion
    if (analysis.systemContext.dayOfWeek === 'Saturday' || analysis.systemContext.dayOfWeek === 'Sunday') {
      suggestions.push({
        id: `suggest-${Date.now()}-13`,
        suggestion: 'Weekend detected. Consider if this task should wait until the work week.',
        contextType: 'system',
        priority: 'low',
        confidence: 55,
  ...
  const generateCrossContextSuggestions = (analysis: ContextAnalysis): ContextAwareSuggestion[] => {
    const suggestions: ContextAwareSuggestion[] = [];
    const now = new Date();
    const hours = now.getHours();
    ...
    if (analysis.currentTaskContext.task &&
      ...
      suggestions.push({
        ...
        priority: 'high',
        confidence: 90,
        reasoning: 'Alignment of high-priority task with user peak productivity time',
        actionType: 'optimization'
      });
    }
    ...
    if (analysis.projectContext.project &&
      ...
      suggestions.push({
        ...
        priority: 'high',
        confidence: 85,
        reasoning: 'High-priority project aligned with optimal system conditions',
        actionType: 'optimization'
      });
    }
    ...
    if (analysis.currentTaskContext.task &&
        ...
        analysis.currentTaskContext.task.priority !== analysis.projectContext.project.priority) {
      suggestions.push({
  ...
  const getContextTypeColor = (contextType: string) => {
    switch (contextType) {
      case 'task': return '#4285F4'; // Blue
      case 'project': return '#0F9D58'; // Green
      case 'user': return '#DB4437'; // Red
      case 'system': return '#F4B400'; // Yellow
      case 'cross-context': return '#9C27B0'; // Purple
    ...
    return (
      <div className="ai-context-assistant">
        ...
        <div className="loading-indicator">
          ...
          <div>Gathering context from tasks, projects, user patterns, and system status...</div>
        </div>
      </div>
    );
  }
        ...
        <div className="error-message">
          ...
          <button
            onClick={() => {
              ...
              else if (mode === 'project' && projectId) analyzeProjectContext(projectId);
        ...
        <div className="context-suggestions">
          {suggestions.length === 0 ? (
            <div className="no-suggestions">
              ...
              <div className="no-suggestions-text">No context-aware suggestions available</div>
              <div className="no-suggestions-subtext">
                {mode === 'task' ? 'Task appears to be well-aligned with current context' :
                ...
                >
                  <div className="suggestion-header">
                    <div
                      ...
                      style={{ backgroundColor: getContextTypeColor(suggestion.contextType) }}
                      title={`Context: ${suggestion.contextType}`}
                    >
                      {suggestion.contextType === 'task' ? '📋' :
                       suggestion.contextType === 'project' ? '🗂️' :
                       suggestion.contextType === 'user' ? '👤' :
                       suggestion.contextType === 'system' ? '🖥️' : '🔗'}
                    </div>
                  ...
                  <div className="suggestion-content">
                    ...
                    <div className="suggestion-meta">
                      ...
                      <span
                        ...
                        style={{ color: getContextTypeColor(suggestion.contextType) }}
                      >
            ...
            {contextAnalysis?.currentTaskContext.task ? (
              <div className="analysis-content">
                <div className="analysis-item">
                  ...
                  <span className="item-value">{contextAnalysis.currentTaskContext.task.title}</span>
                </div>

                <div className="analysis-item">
                  <span className="item-label">Priority:</span>
                  <span className="item-value">
                    <span className="priority-badge" style={{ backgroundColor: getPriorityColor(contextAnalysis.currentTaskContext.task.priority) }}>
                ...
                <div className="analysis-item">
                  ...
                  <span className="item-value">{contextAnalysis.currentTaskContext.task.status}</span>
                </div>

                <div className="analysis-item">
                  <span className="item-label">Dependencies:</span>
                  <span className="item-value">
                    {contextAnalysis.currentTaskContext.taskDependencies.length} blocking,
                    {contextAnalysis.currentTaskContext.dependentTasks.length} depending
                  </span>
                </div>
              </div>
            ) : (
            ...
            {contextAnalysis?.projectContext.project ? (
              <div className="analysis-content">
                <div className="analysis-item">
                  ...
                  <span className="item-value">{contextAnalysis.projectContext.project.name}</span>
                </div>

                <div className="analysis-item">
                  <span className="item-label">Priority:</span>
                  <span className="item-value">
                    <span className="priority-badge" style={{ backgroundColor: getPriorityColor(contextAnalysis.projectContext.project.priority) }}>
                ...
                <div className="analysis-item">
                  <span className="item-label">Status:</span>
                  <span className="item-value">{contextAnalysis.projectContext.project.status}</span>
                </div>
                ...
                <div className="analysis-item">
                  ...
                  <span className="item-value">
                    ...
                    {contextAnalysis.projectContext.projectTasks.filter(t => t.status === 'pending').length} pending
                  </span>
                </div>
              </div>
            ) : (
              <div className="no-project-context">No specific project context</div>
            )}
          </div>
          ...
          <div className="analysis-section">
            ...
            <div className="analysis-content">
              <div className="analysis-item">
                ...
                <span className="item-value">{contextAnalysis?.userContext.currentFocus}</span>
              </div>

              <div className="analysis-item">
                <span className="item-label">Productivity Pattern:</span>
                <span className="item-value">{contextAnalysis?.userContext.productivityPattern}</span>
              </div>

              <div className="analysis-item">
                <span className="item-label">Recent Activity:</span>
                <span className="item-value">{contextAnalysis?.userContext.recentActivity.join(', ')}</span>
              </div>
            </div>
          </div>
          ...
          <div className="analysis-section">
            ...
            <div className="analysis-content">
              <div className="analysis-item">
                ...
                <span className="item-value">{contextAnalysis?.systemContext.currentTime}</span>
              </div>

              <div className="analysis-item">
                <span className="item-label">Day of Week:</span>
                <span className="item-value">{contextAnalysis?.systemContext.dayOfWeek}</span>
              </div>

              <div className="analysis-item">
                <span className="item-label">System Load:</span>
                <span className="item-value">
                  <span className="load-badge" style={{
                    backgroundColor: contextAnalysis?.systemContext.systemLoad === 'high' ? '#ff4444' :
                                      contextAnalysis?.systemContext.systemLoad === 'medium' ? '#ffbb33' : '#00C851'
                  }}>
