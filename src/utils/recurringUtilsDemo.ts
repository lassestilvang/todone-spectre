...
export const demoGenerateRecurringInstances = (): void => {
  ...
  instances.forEach((instance, index) => {
    console.log(
      `  ${index + 1}. ${instance.date.toDateString()} (${instance.isGenerated ? "Generated" : "Original"})`,
    );
  });
};
...
export const demoValidateRecurringConfig = (): void => {
  ...
  console.log(
    `✅ Valid config: ${validResult.valid} (${validResult.errors.length} errors)`,
  );

  const invalidResult = validateRecurringTaskConfig(invalidConfig);
  console.log(
    `❌ Invalid config: ${invalidResult.valid} (${invalidResult.errors.length} errors)`,
  );
