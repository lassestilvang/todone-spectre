...
export const generateMockTasks = (count: number = 5): Task[] => {
  const statuses: TaskStatus[] = ['todo', 'in-progress', 'completed', 'archived'];
 ...
 */
export const mockTaskApiResponse = <T,>(data: T, success: boolean = true, message: string = 'Success'): { success: boolean; message: string; data: T } => {
  return {
    success,
    message,
    data
...
export const taskComparisonTestCases = [
  ...
  },
  {
    ...
    tasks: [
      generateMockTask({ dueDate: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000) }), // 3 days from now
      generateMockTask({ dueDate: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000) }), // 1 day from now
      generateMockTask({ dueDate: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000) })  // 2 days from now
    ],
