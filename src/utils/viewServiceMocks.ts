...
export class ViewServiceMocks {
  ...
  static mockListViewService(): jest.Mocked<ListViewService> {
    return {
      transformTasksForListView: jest.fn((tasks: Task[]) => tasks.map(task => ({
        ...task,
  ...
  static mockBoardViewService(): jest.Mocked<BoardViewService> {
    return {
      transformTasksForBoardView: jest.fn((tasks: Task[]) => tasks.map(task => ({
        ...task,
        displayTitle: task.title || 'Mock Task',
  ...
  static mockCalendarViewService(): jest.Mocked<CalendarViewService> {
    return {
      transformTasksForCalendarView: jest.fn((tasks: Task[]) => tasks.map(task => ({
        ...task,
        displayTitle: task.title || 'Mock Task',
      ...
      getTasksForDateRange: jest.fn((tasks: Task[], startDate: Date, endDate: Date) => {
        return tasks.filter(task => {
      ...
      getDateRangeForNavigation: jest.fn((currentDate: Date, viewMode: string = 'week') => {
        if (viewMode === 'day') {
          return {
