...
export function isTeamMember(team: CollaborationTeam, userId: string): boolean {
  return (
    team.members?.some((m) => m.userId === userId) || isTeamOwner(team, userId)
  );
}
...
export function canInviteMembers(
  ...
  if (hasAdminPrivileges(team, userId)) {
    return (
      team.settings.permissionSettings.allowMemberInvites ||
 ...
 */
export function getTeamActivityStats(
  team: CollaborationTeam,
 ...
 */
export function getUserStatusInTeam(
  team: CollaborationTeam,
  userId: string,
): CollaborationMember["status"] | null {
  const member = team.members?.find((m) => m.userId === userId);
 ...
 */
export function formatPrivacySetting(
 ...
 */
export function getMemberCountByRole(
  team: CollaborationTeam,
  role: CollaborationMember["role"],
): number {
 ...
 */
export function getMemberCountByStatus(
  team: CollaborationTeam,
  status: CollaborationMember["status"],
): number {
 ...
 */
export function validateTeamDescription(description: string): {
  valid: boolean;
  message?: string;
} {
 ...
 */
export function validateMemberRole(role: CollaborationMember["role"]): {
  valid: boolean;
  message?: string;
} {
  const validRoles: CollaborationMember["role"][] = [
    "admin",
    "member",
    "guest",
  ];
 ...
 */
export function validateMemberStatus(status: CollaborationMember["status"]): {
  valid: boolean;
  message?: string;
} {
  const validStatuses: CollaborationMember["status"][] = [
    "active",
    "inactive",
    "pending",
  ];
...
export function validatePrivacySetting(
  ...
  const validSettings: CollaborationTeam["privacySetting"][] = [
    "public",
    "private",
    "team-only",
  ];
 ...
 */
export function getFormattedLastActiveDate(
 ...
 */
export function getMembersSortedByRole(
  team: CollaborationTeam,
): CollaborationMember[] {
 ...
 */
export function getMembersSortedByJoinDate(
  team: CollaborationTeam,
): CollaborationMember[] {
 ...
 */
export function getMembersSortedByLastActive(
  team: CollaborationTeam,
): CollaborationMember[] {
  return [...(team.members || [])].sort((a, b) => {
 ...
 */
export function getMembersSortedByName(
  team: CollaborationTeam,
): CollaborationMember[] {
  return [...(team.members || [])].sort((a, b) => {
 ...
 */
export function isInactiveTeam(
  team: CollaborationTeam,
  activities: CollaborationActivity[],
): boolean {
  ...
  const latestActivity = teamActivities.sort(
...
export function getTeamHealthRecommendation(score: number): string {
  ...
  if (score >= 60)
    return "Your team is doing well. Consider encouraging more participation.";
  if (score >= 40)
    return "Your team could be more active. Try to engage members with new activities.";
  if (score >= 20)
    return "Your team needs more engagement. Consider team-building activities.";
 ...
 */
export function getTeamAccessLevel(
  team: CollaborationTeam,
  userId: string,
 ...
 */
export function hasIntegrationEnabled(
  team: CollaborationTeam,
  integration: keyof CollaborationSettings["integrationSettings"],
): boolean {
...
export function hasNotificationEnabled(
  team: CollaborationTeam,
  notification: keyof CollaborationSettings["notificationSettings"],
): boolean {
 ...
 */
export function getNotificationPreferencesSummary(
  team: CollaborationTeam,
): string {
...
export function getPermissionSettingsSummary(team: CollaborationTeam): string {
  ...
  if (settings.requireAdminApproval)
    permissions.push("Admin approval required");
...
export function getSettingsComparison(team: CollaborationTeam): {
  ...
  if (
    JSON.stringify(team.settings.notificationSettings) !==
  ...
  if (
    JSON.stringify(team.settings.permissionSettings) !==
    JSON.stringify(defaultSettings.permissionSettings)
  ) {
  ...
  if (
    JSON.stringify(team.settings.privacySettings) !==
    JSON.stringify(defaultSettings.privacySettings)
  ) {
  ...
  if (
    JSON.stringify(team.settings.integrationSettings) !==
 ...
 */
export function getTeamDetailedSummary(
  team: CollaborationTeam,
  activities: CollaborationActivity[],
): string {
  ...
  return (
    `${team.name} - ${healthStatus} health (${healthScore}/100)\n` +
