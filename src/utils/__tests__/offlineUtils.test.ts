...
describe('Offline Utilities Integration Tests', () => {
  ...
  describe('Data Transformation Utilities', () => {
    ...
    test('prepareQueueItemData should create valid queue items', () => {
      const queueItem = prepareQueueItemData('test-operation', 'create', testData);
    ...
    test('batch operations should work correctly', () => {
      ...
      const batchRestored = batchTransformOfflineToApplication(batchTransformed);
  ...
  describe('Sync Utilities', () => {
    ...
    test('validateQueueItem should validate correctly', () => {
      const validResult = validateQueueItem('test-op', 'create', testData, testSettings);
      expect(validResult.isValid).toBe(true);

      const invalidResult = validateQueueItem('', 'invalid' as any, null, testSettings);
  ...
  describe('Service Integration', () => {
    test('offlineUtilsService should be accessible', () => {
      ...
      expect(typeof offlineUtilsService.transformDataForOfflineStorage).toBe('function');
    ...
    test('service methods should integrate utilities correctly', () => {
      ...
      const transformed = offlineUtilsService.transformDataForOfflineStorage(testData);
      expect(transformed).toBeDefined();

      // Test settings validation through service
      const validation = offlineUtilsService.validateOfflineSettings(testSettings);
  ...
  describe('Comprehensive Workflow', () => {
    test('complete offline workflow should work', () => {
      ...
      const offlineData = offlineUtilsService.transformDataForOfflineStorage(testData);
      ...
      // 5. Validate settings
      const settingsValidation = offlineUtilsService.validateOfflineSettings(testSettings);
