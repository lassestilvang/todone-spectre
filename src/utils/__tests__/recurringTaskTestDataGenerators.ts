...
export class RecurringTaskTestDataGenerator {
  ...
  public static getInstance(): RecurringTaskTestDataGenerator {
    if (!RecurringTaskTestDataGenerator.instance) {
      RecurringTaskTestDataGenerator.instance = new RecurringTaskTestDataGenerator();
    }
  ...
  public generateRecurringTaskScenarios(): Record<string, Task> {
    return {
      dailyStandup: this.generateRealisticRecurringTask('daily', {
        ...
        description: '15-minute daily team sync to discuss progress and blockers',
        customFields: {
          recurringConfig: {
            pattern: 'daily',
      ...
      weeklyTeamMeeting: this.generateRealisticRecurringTask('weekly', {
        ...
        description: 'Weekly team meeting to discuss project progress and planning',
        customFields: {
          recurringConfig: {
            pattern: 'weekly',
            startDate: startOfDay(new Date()),
  ...
  public generatePerformanceTestData(taskCount: number = 100): Task[] {
    ...
    for (let i = 0; i < taskCount; i++) {
      ...
      tasks.push(this.generateRealisticRecurringTask(pattern, {
  ...
  private generateRealisticTaskDescription(pattern: RecurringPattern): string {
    ...
    return descriptions[pattern][Math.floor(Math.random() * descriptions[pattern].length)];
  }
  ...
  private generateRealisticRecurringConfig(pattern: RecurringPattern): RecurringTaskConfig {
  ...
  private getNextInstanceDate(baseTask: Task, instanceNumber: number): Date {
    ...
    const startDate = new Date(config.startDate || baseTask.dueDate || new Date());
    let nextDate = new Date(startDate);

    if (config.pattern === 'daily') {
      nextDate = addDays(nextDate, instanceNumber * (config.customInterval || 1));
    } else if (config.pattern === 'weekly') {
      nextDate = addWeeks(nextDate, instanceNumber * (config.customInterval || 1));
    } else if (config.pattern === 'monthly') {
      nextDate = addMonths(nextDate, instanceNumber * (config.customInterval || 1));
    } else if (config.pattern === 'yearly') {
      nextDate = addYears(nextDate, instanceNumber * (config.customInterval || 1));
...
// Singleton instance
export const recurringTaskTestDataGenerator = RecurringTaskTestDataGenerator.getInstance();