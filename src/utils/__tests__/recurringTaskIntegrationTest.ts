...
export class RecurringTaskIntegrationTest {
  ...
  async runAllTests(): Promise<{
    ...
    console.log(`📈 Success Rate: ${Math.round((passedTests / this.testResults.length) * 100)}%`);
  ...
  private async testValidationAndErrorHandling(): Promise<void> {
    try {
      ...
      this.testResults.push({
        ...
        message: !validation.valid && validation.errors.length > 0 ? 'Invalid config detected' : 'Invalid config not detected'
      });
      ...
      const patternValidation = recurringPatternService.validatePatternConfig(invalidPatternConfig);
      this.testResults.push({
        testName: 'Validation: Pattern Validation',
        success: !patternValidation.valid,
        message: !patternValidation.valid ? 'Invalid pattern detected' : 'Invalid pattern not detected'
      });
