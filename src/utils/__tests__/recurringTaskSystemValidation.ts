...
async function runIntegrationTests(validationResults: any[]): Promise<void> {
  try {
    ...
    if (testDates.length === 6) {
      // 1 original + 5 generated
      validationResults.push({
...
async function runPerformanceTests(validationResults: any[]): Promise<void> {
  try {
    ...
    const complexity =
      recurringPatternManager.getPatternComplexityScore(testConfig);
...
export async function runCompleteRecurringTaskSystemValidation(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `🔹 Overall System Valid: ${result.isValid ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `🔹 Total Components Validated: ${result.validationResults.length}`,
    );
    console.log(
      `🔹 Critical Failures: ${result.validationResults.filter((r) => r.status === "fail").length}`,
    );
    console.log(
      `🔹 Warnings: ${result.validationResults.filter((r) => r.status === "warning").length}`,
    );
    console.log(
      `🔹 Passes: ${result.validationResults.filter((r) => r.status === "pass").length}\\n`,
    );

    console.log("📋 Service Validation:");
    console.log(
      `  RecurringTaskService: ${result.systemValidation.serviceValidation.taskService ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  RecurringPatternManager: ${result.systemValidation.serviceValidation.patternManager ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  RecurringTaskScheduler: ${result.systemValidation.serviceValidation.taskScheduler ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  RecurringTaskGenerator: ${result.systemValidation.serviceValidation.taskGenerator ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  RecurringTaskIntegration: ${result.systemValidation.serviceValidation.taskIntegration ? "✅ PASS" : "❌ FAIL"}\\n`,
    );

    console.log("📋 Store Validation:");
    console.log(
      `  Store Available: ${result.systemValidation.storeValidation.storeAvailable ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  Store Functional: ${result.systemValidation.storeValidation.storeFunctional ? "✅ PASS" : "❌ FAIL"}\\n`,
    );

    console.log("📋 Component Validation:");
    console.log(
      `  RecurringTaskForm: ${result.systemValidation.componentValidation.formComponent ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  RecurringTaskList: ${result.systemValidation.componentValidation.listComponent ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  RecurringTaskPreview: ${result.systemValidation.componentValidation.previewComponent ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  RecurringTaskScheduler: ${result.systemValidation.componentValidation.schedulerComponent ? "✅ PASS" : "❌ FAIL"}\\n`,
    );

    console.log("📋 Hook Validation:");
    console.log(
      `  useRecurringTasks: ${result.systemValidation.hookValidation.useRecurringTasks ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  useRecurringPatterns: ${result.systemValidation.hookValidation.useRecurringPatterns ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  useRecurringTaskState: ${result.systemValidation.hookValidation.useRecurringTaskState ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `  useRecurringTaskIntegration: ${result.systemValidation.hookValidation.useRecurringTaskIntegration ? "✅ PASS" : "❌ FAIL"}\\n`,
    );
    ...
    if (!result.isValid) {
      ...
      result.validationResults
        ...
        .forEach((result) => {
          console.log(
            `  ${result.status === "fail" ? "❌" : "⚠️"} ${result.component}: ${result.message}`,
          );
        });
      console.log();
    }
