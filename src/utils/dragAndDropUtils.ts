...
export const validateDragSource = (source: string, config: DragAndDropConfig = defaultDragAndDropConfig): boolean => {
  return config.allowedSources.includes(source);
};

export const validateDropTarget = (targetId: string, config: DragAndDropConfig = defaultDragAndDropConfig): boolean => {
...
export const parseDragDataTransfer = (data: string): { taskId: string; source: string } | null => {
  try {
