...
import { addDays, addWeeks, addMonths, addYears, isBefore, isAfter, getDay, format } from 'date-fns';
import { recurringPatternService } from '../services/recurringPatternService';

/**
 * Normalize recurring pattern configuration
 */
export const normalizeRecurringPatternConfig = (config: Partial<RecurringPatternConfig>): RecurringPatternConfig => {
  return {
 ...
 */
export const getPatternFrequencyDescription = (config: RecurringPatternConfig): string => {
 ...
 */
const getCustomPatternDescription = (config: RecurringPatternConfig): string => {
  ...
  switch (config.frequency) {
    ...
    case 'weekly':
      if (config.customDays) {
        const dayNames = config.customDays.map(day => getDayName(day).substring(0, 3));
 ...
 */
export const calculatePatternComplexityScore = (config: RecurringPatternConfig): number => {
  let score = 0;
 ...
 */
export const validatePatternConfiguration = (config: RecurringPatternConfig): { valid: boolean; errors: string[] } => {
 ...
 * Get pattern presets
 */
export const getPatternPresets = (): Array<{ id: string; name: string; config: RecurringPatternConfig }> => {
 ...
 */
export const getDefaultPatternConfig = (pattern: RecurringPattern): RecurringPatternConfig => {
 ...
 */
export const formatPatternForDisplay = (config: RecurringPatternConfig): string => {
 ...
 */
export const getEndConditionDescription = (config: RecurringPatternConfig): string => {
 ...
 */
export const calculateNextOccurrenceDate = (currentDate: Date, config: RecurringPatternConfig): Date => {
  return recurringPatternService.generateRecurringDates(currentDate, config, 1)[1]?.date || currentDate;
};
...
export const generatePreviewInstances = (
  ...
  return recurringPatternService.generateRecurringDates(startDate, config, count).map((instance, index) => ({
    id: `preview-${index}`,
    taskId: 'preview',
    date: instance.date,
