import { CalendarEventType, CalendarType, CalendarConfig, CalendarSyncStatus } from '../types/calendarTypes';
...
export class CalendarServiceMocks {
  ...
  static createMockCalendarService() {
    ...
    return {
      ...
      getEventById: async (eventId: string): Promise<CalendarEventType | null> => {
        return new Promise(resolve => {
          setTimeout(() => {
      ...
      createEvent: async (eventData: Omit<CalendarEventType, 'id'>): Promise<CalendarEventType> => {
        return new Promise(resolve => {
          setTimeout(() => {
      ...
      updateEvent: async (eventId: string, updates: Partial<CalendarEventType>): Promise<CalendarEventType | null> => {
        return new Promise(resolve => {
          setTimeout(() => {
      ...
      deleteEvent: async (eventId: string): Promise<boolean> => {
        return new Promise(resolve => {
          setTimeout(() => {
            ...
            mockEvents.splice(mockEvents.findIndex(e => e.id === eventId), 1);
      ...
      addCalendar: async (calendarData: Omit<CalendarType, 'id'>): Promise<CalendarType> => {
        return new Promise(resolve => {
          setTimeout(() => {
      ...
      linkTaskToEvent: async (taskId: string, eventId: string): Promise<boolean> => {
        return new Promise(resolve => {
          setTimeout(() => {
            const event = mockEvents.find(e => e.id === eventId);
            if (event) {
      ...
      getEventsForDateRange: async (startDate: Date, endDate: Date): Promise<CalendarEventType[]> => {
        return new Promise(resolve => {
          setTimeout(() => {
            resolve(mockEvents.filter(event => {
  ...
  static createMockCalendarSyncService() {
    ...
    return {
      syncCalendars: async (calendarIds: string[]): Promise<{
        success: boolean;
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              if (Math.random() > 0.8) { // 20% chance of failure for testing
                ...
                const syncedEvents: CalendarEventType[] = calendarIds.map((calendarId, index) => ({
                  ...
                  endDate: new Date(Date.now() + (index * 86400000) + 3600000).toISOString(),
      ...
      syncTasksWithCalendarEvents: async (tasks: Task[]): Promise<{
        ...
        return new Promise(resolve => {
          setTimeout(() => {
            ...
            tasks.forEach((task, index) => {
              if (Math.random() > 0.7) { // 30% chance of error for testing
