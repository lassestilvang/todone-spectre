...
export const validatePerformanceConfig = (config: Partial<PerformanceConfig>): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  if (config.memoryThreshold !== undefined) {
    if (typeof config.memoryThreshold !== 'number' || config.memoryThreshold <= 0) {
...
export const mergePerformanceConfigs = (baseConfig: PerformanceConfig, overrideConfig: Partial<PerformanceConfig>): PerformanceConfig => {
...
export const serializePerformanceConfig = (config: PerformanceConfig): string => {
  return JSON.stringify(config, null, 2);
};

export const deserializePerformanceConfig = (serialized: string): PerformanceConfig | null => {
  try {
...
export const areConfigsEqual = (config1: PerformanceConfig, config2: PerformanceConfig): boolean => {
