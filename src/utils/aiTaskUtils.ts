...
export class AITaskUtils {
  ...
  static generateActionableItems(task: Task, count: number = 3): ActionableItem[] {
  ...
  private static generateStepsForTask(task: Task, complexityLevel: string): string[] {
    const baseSteps = [
  ...
  private static estimateTaskTime(task: Task, complexityScore: number): string {
    ...
    const descLengthFactor = task.description ? Math.min(task.description.length / 20, 2) : 0;

    const timeMultiplier = 1 + titleLengthFactor + descLengthFactor + (complexityScore / 50);
  ...
  private static mapConfidenceToPriority(confidence: number): 'low' | 'medium' | 'high' {
  ...
  private static getAssistanceLevel(complexityLevel: string): 'low' | 'medium' | 'high' {
