 ...
 */
export const createQuickActionConfig = (action: Partial<QuickAction> & { id: string }): QuickAction => ({
  id: action.id,
  label: action.label || action.id.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
 ...
 */
export const configToStorageFormat = (config: ExtensionConfig): Record<string, any> => ({
  ...config,
 ...
 */
export const configFromStorageFormat = (storageData: Record<string, any>): ExtensionConfig => {
...
export const getConfigDifferences = (
  ...
  if (JSON.stringify(oldConfig.quickActions) !== JSON.stringify(newConfig.quickActions)) {
 ...
 */
export const getThemeCssVariables = (theme: ExtensionConfig['theme']): Record<string, string> => {
  switch (theme) {
    case 'light':
      return {
 ...
 */
export const applyThemeToElement = (element: HTMLElement, theme: ExtensionConfig['theme']): void => {
 ...
 */
export const createThemeStylesheet = (theme: ExtensionConfig['theme']): HTMLStyleElement => {
 ...
 */
export const getConfigValidationErrors = (config: Partial<ExtensionConfig>): string[] => {
  const errors: string[] = [];
  ...
  if (config.theme !== undefined && !['system', 'light', 'dark'].includes(config.theme)) {
 ...
 */
export const restoreConfigFromBackup = (backupData: string): ExtensionConfig => {
  try {
