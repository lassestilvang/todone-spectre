...
export const createDragEventData = (taskId: string, source: string, clientX: number, clientY: number): DragEventData => {
  return {
    taskId,
    source,
    clientX,
...
export const getDragPosition = (event: React.DragEvent | MouseEvent): { x: number; y: number } => {
...
export const updateDragPreviewPosition = (preview: HTMLElement, x: number, y: number) => {
